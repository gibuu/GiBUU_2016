!***************************************************************************
!****m* /formfactors_A_piZero_proton
! NAME
! module formfactors_A_piZero_proton
!
! PURPOSE
! This module reads in the form factors for gamma* N -> N pi out of text files
...
contains
  ...
  !*************************************************************************
  !****f* formfactors_A_piZero_proton/getA
  ! NAME
  ...
  !*************************************************************************
  function getA_piZero_proton (thetaIn,sIn,QSquaredIn) result(getA)
    ...
    if (initFlag) then
       call Write_InitStatus('MAID Amplitudes for gamma p -> p pi0',0)
       call init_datFile_pi0p
       initFlag=.false.
       ...
       call Write_InitStatus('MAID Amplitudes for gamma p -> p pi0',1)
    end if
  ...
  contains
    ...
    subroutine init_datFile_pi0p
      ...
      f = bzOpenR(trim(get_Filename('pi0p')))
      ...
      if(debug) then
         ! Control output
         open(101,file='FF_A_pi0_proton.dat')
    ...
    end subroutine init_datFile_pi0p
  end function getA_piZero_proton
...
end module formfactors_A_piZero_proton
