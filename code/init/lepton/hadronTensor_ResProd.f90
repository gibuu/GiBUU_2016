...
contains
  ...
  subroutine initInput
    ...
    ! * speedup
    !*************************************************************************
    NAMELIST /HadronTensor_ResProd/ debugFlag,speedup
  ...
  !***************************************************************************
  function hadronTensor_R(pi,pf,resID,targetCharge,process,matrix,bare_mass)   result(success)
    ...
    CASE(1)
       ! e.g. S11,P11
       matrix=hadronTensor_1_2(pi,pf,parity,formfactor(1:2),formfactor(5:6),resID,processID)
    CASE(3)
       ! e.g. D13,P33
  ...
  function hadronTensor_1_2(pi,pf,parity,G,GA,resID,process) result(matrix)
    ...
    complex, dimension(0:3,0:3) :: projector_in,projector_out,j_mu,j_nu,j_dagger_nu
    real                        :: mi,mf
    integer                     :: mu,nu
!    logical, parameter          :: speedup=.true.
    ...
    do mu=0,3
       ...
       do nu=0,3
          ...
          if(speedup) then
             ...
             matrix(mu,nu)=1./2.* &
                  & Trace(MatrixMult(projector_out,j_mu,projector_in,gamma0,dagger(j_nu),gamma0))
          end if
  ...
  contains
    ...
    function j_neg(mu) result(matrix)
      ...
      if(do_once.or.(.not.speedup)) then
         q= pf-pi
         ! This definition differs in comparison to Lalakulich, is however consistent with Alvarez-Ruso
         mass_mu=2*mN
         QSquared=-SP(q,q)
         if(speedup) then
            slashed_q=slashed(q)
      ...
      if(speedup) then
         ...
         matrix= MatMul(  G(1)/mass_mu**2           * (QSquared*gamma(:,:,mu)+q(mu)*slashed(q)) &
  ...
  function   hadronTensor_3_2(pi,pf,parity,F,resID,process) result(matrix)
    ...
    complex, dimension(0:3,0:3) :: projector_in,projector_out,j_mu,j_nu, matrix_toTrace,j_nu_dagger
             ...
             betaSum : do beta=0,3
                ...
                kappaSum : do kappa=0,3
                   ...
                   deltaSum : do delta=0,3
                      ...
                      if(speedup) then
                         j_mu=j_neg(delta,mu)
                         matrix_toTrace=matrix_toTrace+ metricTensor(beta,delta)*metricTensor(alpha,kappa)*&
                         ...
                         matrix_toTrace=matrix_toTrace+ metricTensor(beta,delta)*metricTensor(alpha,kappa)*&
                              & MatrixMult(projector_out,j_mu,projector_in,gamma0,dagger(j_nu),gamma0)
                      end if
                   end do deltaSum
                end do kappaSum
             end do betaSum
  ...
  contains
    ...
    function j_neg(lambda,nu) result(matrix)
      ...
      if(do_once2.or.(.not.speedup)) then
         ...
         !mf=abs4(pf)
         if(speedup) then
            matrix_1= F(1)/mN * slashed(q) + (F(2)/mN**2 * SP(pf,q)+ F(3)/mN**2 * SP(pi,q) + F(4))* unit4
            F1_mN=F(1)/mN
            F2_mN2=F(2)/mN**2
            F3_mN2=F(3)/mN**2
            if(process.ne.EM) then
     ...
     if(process.ne.EM) then
        if(speedup) then
           matrix=matrix &
                ...
                & - q(lambda)*               ( F5_mN * MatMul(gamma(:,:,nu) ,gamma5) + F6_mN2 * pf(nu)  * gamma5  )   &
                & + (metricTensor(lambda,nu)* F(7)+q(lambda)*q(nu)*F8_mN2)*gamma5
         else
            matrix=matrix &
                 + metricTensor(lambda,nu)* ( F(5)/mN * MatMul(slashed(q),gamma5) + F(6)/mN**2* SP(pf,q) * gamma5 ) &
                 - q(lambda)*               ( F(5)/mN * MatMul(gamma(:,:,nu) ,gamma5) + F(6)/mN**2 * pf(nu) * gamma5 )&
    ...
    function j_dagger(lambda,nu) result(matrix)
     ...
     if(process.ne.EM) then
           matrix=matrix &
                & + metricTensor(lambda,nu)* matrix_2  &
                & + q(lambda)*               ( F5_mN * MatMul(gamma5,gamma(:,:,nu)) + F6_mN2 * pf(nu)  * gamma5    )   &
                & - (metricTensor(lambda,nu)* F(7)+q(lambda)*q(nu)*F8_mN2)*gamma5
     end if
   end function j_dagger
