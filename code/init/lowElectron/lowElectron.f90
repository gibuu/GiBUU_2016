...
contains
  ...
  subroutine init_lowElectron(realParticles,pertParticles,raiseEnergy,targetNuc)
    ...
    use eN_eventDefinition , only : electronNucleon_event, CM, CALC
    use eN_event           , only : eNev_SetProcess, init_electronNucleon_event, nuclearFluxFactor_correction
...
!!$          end if
  ...
  subroutine initInput
    ...
    use leptonKinematics, only : evaluate_QSquared,evaluate_theta_lf,evaluate_energy_lf
