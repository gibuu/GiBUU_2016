  ...
  contains
    ...
    subroutine setVelocity
      ...
      if (cmsFlag) then
         ...
         If (debug) Print *, 'Momentum in CM = ', momentumCM
         ...
         If (debug) then
            Print *, 'Projectile momentum in LAB = ',momentumProjectileLab
            Print *, 'Target momentum in LAB = ',momentumTargetLab
         End if
    ...
    subroutine setPosition
      ...
      If (distance < distance_touch) then
         Write (*,*) 'Heavy Ions are sitting too close at initilization'
