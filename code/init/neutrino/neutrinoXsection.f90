...
module neutrinoXsection
  ...
  !
  ! In case of electron induced events, we need choise 2 in order to be
...
contains
  ...
  subroutine readinput(nuXsectionmode)
    ...
    case (1)
       write(*,'(A)') ' DIS form factor (EM)   : 1,  = Q^2/(m_\rho^2+Q^2)'
    case (2)
       write(*,'(A)') ' DIS form factor (EM)   : 2,  = Q^4/(m_\rho^2+Q^2)^2'
    case default
    ...
    case (1)
       write(*,'(A)') ' DIS form factor (CC,NC): 1,  = Q^2/(m_\rho^2+Q^2)'
    case (2)
       write(*,'(A)') ' DIS form factor (CC,NC): 2,  = Q^4/(m_\rho^2+Q^2)^2'
    case default
  ...
  subroutine Xsec_SigmaMC(eNev, IP, raiseFlag, raiseVal, &
    ...
    use neutrino_IDTable
    use eN_event, only: eNev_init_nuStep1,eNev_init_nuStep2,eNev_init_nuStep3a
  ...
  subroutine SetXsecMC(eNev, flux_enu, XsectionMode)
    use random, only: rn
    use eN_event, only: eNev_init_nuStep2,eNev_init_nuStep3b,eNev_init_nuStep3c
    use neutrino_IDTable
  ...
  subroutine Xsec_dSigmadQsdElepton(eNev, IP, raiseFlag, raiseVal, &
    ...
    use neutrino_IDTable
    use idtable, only : nucleon
    use eN_event, only : eNev_init_nuStep1,eNev_init_nuStep2,eNev_init_nuStep3b
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine XsecdCosthetadElepton(eNev,IP,OutPart,sig)
    ...
    real :: kin_factor,invMassSQ,Q2,Wrec  !,Wrec2,W2,Wmed
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    case(DIS_CH)
       ...
       case(-1,1) !=== EM
          select case (DISformfakEM)
          case (1)
             sig = sig * eNev%Qsquared/(eNev%Qsquared+0.6**2)
          case (2)
...
! IP=35 = 2p2h process
  ...
  subroutine get_xsection_namelist(XsectionMode,Gdebugflag,Gnuclear_phasespace,GsinglePiModel,GintegralPrecision, &
    ...
    if (present (XsectionMode)) then
        if(initflag) then
           call readInput(MOD(XsectionMode,10))
           initflag = .false.
        end if
    end if
