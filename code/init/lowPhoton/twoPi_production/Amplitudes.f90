...
contains
  ...
  subroutine init
    ...
    NAMELIST /Amplitudes_2Pi/ inMedium_delta_width, inMedium_delta_potential, inMedium_nucleon_potential, &
                              inMedium_pion_potential, debugFlag, buuPotential
  ...
  ! L. Alvarez-Ruso
  !*****************************************************************************
 subroutine ampli(r,q,p1,p2,p4,p5,o,t,betaToLRF,mediumAtPosition,position)
   ...
   real, dimension(0:3), intent(in):: p1 ! incoming nucleon momentum
   real, dimension(0:3), intent(in):: p2 ! outgoing nucleon momentum
   real, dimension(0:3), intent(in):: p4 ! outgoing pi(- or 0) momentum
   real, dimension(0:3), intent(in):: p5 ! outgoing pi(+ or 0) momentum
   ...
   real:: fo
   real:: f1,gm,ed,mud,g1,g2,g3,f1rop,f2rop

   real :: smandel
   real, parameter :: GEV_TO_INVfm=1000./197.

   !     Some propagators deppend only at s so it is convenient to calculate them
   ...
   complex function gn(p)
     ...
     if(inMedium_nucleon_potential) then
        ...
        If(buuPotential) then
           ...
           selfenergy=-0.05*Gev_To_INVFM*(mediumAtPosition%DensityNeutron+mediumAtPosition%DensityProton)/0.168
           If(debugFlag) Print *, 'self=', selfenergy
        end if
     ...
     If(inMedium_delta_width) then
        ...
        If(inMedium_delta_potential) then
           ...
           If(buuPotential) then
              ...
              selfenergy=-0.03*Gev_To_INVFM*(mediumAtPosition%DensityNeutron+mediumAtPosition%DensityProton)/0.168
              If(debugFlag) Print *, 'self=', selfenergy,selfEnergy/GeV_TO_InvFm
           end if
        else
           selfenergy=0.
        end if
