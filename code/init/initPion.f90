  ...
  contains
    ...
    subroutine initInput(outside)
      ...
      NAMELIST /pionNucleus/ UseCoulomb,CoulombDistance,distance,&
           & impact_parameter,&
           & charge,numberPions,ekin_lab,delta_ekin_lab, debug
    ...
    subroutine RechneImpuls(teilchenIn,energy,success)
      ...
      Do j=0,maxSteps
         ...
         Do i=-1,1
            ...
            teilchen%momentum(3)=teilchenIn%momentum(3)+i*dp
            call energyDetermination(teilchen)
            f(i)=teilchen%momentum(0)-energy
         end do
         if (Abs(f(0)).lt.0.003) then
            If (debug) then
               Print *, 'Kinetic Energy in medium:',kineticEnergy(teilchenIn)
               Print *, 'Kinetic Energy in vacuum:',ekin_lab
               Print *, 'Position=',teilchenIn%position
            End if
            success=.true.
            return
         end if
         ...
         If (Abs(grad).lt.0.0001) then
            Print *, "Gradient zero in RechneImpuls von initPion", grad
            Print *, "Energy", energy
            Print *, "Momentum", teilchen%momentum
            Print *, "Step ", j
            Print *, f(-1),f(0),f(1)
            success=.false.
            return
         end if
         teilchenIN%momentum(1:2)=0.
         teilchenIN%momentum(3)=teilchenIN%momentum(3)-f(0)/grad
         call energyDetermination(teilchen)
      End do
      Print *, "Fehler in initpion.f90,RechneImpuls",f(0)
      success=.false.
