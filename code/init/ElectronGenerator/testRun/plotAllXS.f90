...
program plotAllXS
  ...
  Type(electronNucleon_event), save :: eNev_InitData1,eNev_InitData2
...
!  call InitParticleProperties
  ...
  do iQ2=iQ2min,iQ2max,iQ2delta
     ...
     do iW=iWmin,iWmax,iWdelta
        ...
        eNev_InitData1 = eNev_init_eWQ(eps1,W,Q2)
        eNev1 = eNev_init_Target(eNev_InitData1,TargetNuc,flagOK)

        eNev_InitData2 = eNev_init_eWQ(eps2,W,Q2)
        eNev2 = eNev_init_Target(eNev_InitData2,TargetNuc,flagOK)

        call write_electronNucleon_event(eNev1,.FALSE.,.FALSE.) 
        call eNeV_GetKinV(eNev1, nu,Q2,W,Wfree,eps,fT)
        s1=abs4(eNev1%nucleon%momentum+eNev1%electron_in)
        ...
        s2=abs4(eNev2%nucleon%momentum+eNev2%electron_in)
...
!        stop
        ...
        if (iN1.gt.0) then
           ...
           if (isHigh) then
              ...
              fT = fT/ ( 1e3* pi/(eNev1%electron_out(0)*eNev1%electron_in(0)))
           endif
        ...
        if (iN2.gt.0) then
           ...
           if (isHigh) then
              ...
              fT = fT/ ( 1e3* pi/(eNev2%electron_out(0)*eNev2%electron_in(0)))
           endif
