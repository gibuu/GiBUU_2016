...
contains
  ...
  real function scapot1(part,baremass_out,success, addCoulomb)
    ...
    if (.not.flagOK) then
       ...
       if(debugflag) then
          write(*,*) 'final state mass less than zero!',abs4Sq(p),' count = ',zerocount
          write(*,*) 'pot=',vecpot
  ...
  !*************************************************************************
  real function trueEnergy (teilchenIn)
    use particleDefinition
    use mediumDefinition
    ...
    if (isBaryon(Teilchen%Id)) then
       med = mediumAt (teilchen%position(1:3))  ! evaluate density
       call boostToLRF(teilchen,1)  ! boost from calculation frame to LRF
       teilchen%momentum(0) = teilchen%momentum(0) + rearrangementPotential (teilchen, med)
       call boostToLRF(teilchen,2)  ! boost from LRF to calculation frame
       trueEnergy=teilchen%momentum(0)
    else if (isMeson(Teilchen%Id)) then
       ! free energy in calculation frame:
       trueEnergy=teilchen%momentum(0)
    else
       trueEnergy=0.
       write(*,*) 'Funny particle in trueEnergy. ID=',Teilchen%ID
       stop
    End if
  ...
  !**************************************************************************
  subroutine massDetermination_noPart(ID,momentumLRF,med,baremass,verbose,success,pos)
    use particleDefinition
    use mediumDefinition
    ...
    if(bareMassSquared.lt.0) then
       if(verbose_) then
          ...
          write(*,'(A,I3,A,4E15.4,A,2F9.3,A,F9.4)') 'ID:', ID, &
               & 'p=',momentumLRF,'Dens=',med%DensityProton,med%DensityNeutron,'pot=',pot
       end if
       baremass=0.
       if(present(success)) success=.false.
    else
  ...
  !-------------------------------------------------------------------------
  subroutine massDetermination_part (teilchenIn, betaToCF, verbose, success)
    ...
    type(particle), intent(inOut) :: teilchenIn
    ...
    If (DebugFlag) Print *, '**In MassDetermination'
    ...
    ! Check input
    If (AbsMom(teilchenIn)>1000.) then
       call errorMessage
       if (isHadron(teilchen%ID)) then
          teilchenIn%mass=hadron(teilchen%ID)%mass
       else
          teilchenIn%mass=0.
       end If
       return
    ...
    if (h<0.) then
       ...
       if (.not.present(success)) then
          ...
          teilchenIn%ID=0 ! delete particle
       end if
    else
       if (isMeson(teilchen%ID)) then
          med = mediumAt(teilchen%position)
       ...
       teilchenIn%mass = sqrt(max(0.,h-rp))
       if (present(success)) success=.true.
    end if
  ...
  contains
    ...
    subroutine errorMessage
      ...
      write(*,*) 'Charge:       ', teilchenIn%charge
      write(*,*) 'Bare mass:    ', teilchenIn%mass
      write(*,*) 'Eff. mass:    ', abs4(teilchenIn%momentum)
      write(*,*) 'Momentum:     ', teilchenIn%momentum
      write(*,*) 'Position:     ', teilchenIn%position
      write(*,*) 'Perturbative: ', teilchenIn%perturbative
      write(*,*) 'Offshell par: ', teilchenIn%offshellParameter
      dens = densityAt(teilchenIn%position)
