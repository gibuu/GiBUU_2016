...
contains
  ...
  real function symEn_nuc (Q, med) result (esymp)
    ...
    case(4)
      ...
      esymp = esymp / 1000.
    case default
      print *,'symmetryPotFlag: invalid value!', symmetryPotFlag
      call traceback()
    end select
  ...
  real function symEn_Delta (Q, med) result (esymp)
    ...
    case default
      print *,"symEn_Delta: wrong Delta charge!", Q
      call traceback()
    end select
  ...
  real function BaryonPotential (teilchen, med, positionNotSet, EQS_in)
    ...
    case(6)
       if (positionNotSet) call TRACEBACK('the position of the particle must be known')
...
!          out = out + momentumDependentPart(pAbs,c(EQS98MomDep),lambda(EQS98MomDep),med%density)
  ...
  real function variableSkyrme(rho,p)
    ...
    if(notInitialized) then
       ...
       call evaluate_skyrmeParameters (rhoNull_GeV, p_0, u_0, bindingEnergy, compressibility,A,B,C,tau,lambda,success)
       if(.not.success) then
...
!    LDApotential=(2.*b1*rho+7./3.*b2*rho**(4./3.)+8./3.*b3*rho**(5./3.)-2*a*rhoLaplace(teilchen%position,stepsize)-e0f*E0)*hbarc/1000.
  ...
  real function rearrangementPotential (teilchen, med)
    ...
    case(1:5,9:12)
       ...
       if (SurfacePotFlag) skyrme_b=skyrme_b + SurfacePart(teilchen,eta(EQS_Type))/2.
