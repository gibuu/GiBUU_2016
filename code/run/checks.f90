...
!   end subroutine ChecksSwitchGS
  ...
  subroutine evaluateFermiSurface(fileName)
    ...
    if(densInt.gt.1E-10)   write(100,*) '# Average surface energy:', average/densInt
    ...
    if(densInt.gt.1E-10)  write(100,*) '# Average single-partilce binding energy:', average/densInt
...
!   end subroutine checkPauli
  ...
  subroutine checkMomentumDensity(realParticles,timestep,filename)
    ...
    if (DoInit) then
       ...
       call CreateHist2D(momDens,'momentum vs. density distribution',(/0.,0./),(/0.4,0.18/),(/dp,0.005/))
  ...
  !*************************************************************************
  subroutine checkDensity(realParticles,filename)
    use densityModule, only : densityAt,get_densitySwitch,set_densitySwitch,updateDensity,gridPoints,gridSpacing
    use dichtedefinition
    use particleDefinition
    ...
    Do j=-30,30
       ...
       Do i=-30,30
          ...
          write(121,fmt='(5f8.3)') x,z,density%baryon(0),density%proton(0),density%neutron(0)
       end do
       write(121,*)
    End do
    Close(121)
    ...
    write(*,*) 'meanvalue of baryon density=', meanValue/float(2*gridPoints(1))/float(2*gridPoints(2))/float(2*gridPoints(3))
    ...
    Do j=-80,80
       ...
       place=(/0.,0.,z/)
       density=densityAt(place)
       write(121,fmt='(5f8.3)') z,density%baryon(0),density%proton(0),density%neutron(0)
    End do
    Close(121)
...
!   end subroutine CheckPot
  ...
  subroutine checkRadius(teilchen, time)
    ...
    if (PRESENT(time)) then
       write(20,fmt='(3F14.5,I12,2F14.5)') time,Radius,sqrt(Radius2),numberNucleons,minimalRadius,maximalRadius
    else
...
!   end function saveEpot
  ...
  subroutine checkEnergyLDAWelke(teilchen)
    ...
    ! Ausgabe von Zeitschritt, Gesamtenergie, kinetischer Energie, Potentialenergie, Gradientenenergie, Anzahl der Teilchen,
    ! Anzahl der ungebundenen Teilchen, Teilchen die über den maximalen Radius der berechneten Dichtevertielung hinaus sind,
    ! dasselbe +1fm, +2fm, Energie durch Impulsdifferenz
    write(20,fmt='(I5,1X,F10.4,1X,F10.4,1X,F10.4,1X,F10.4,1X,I7,1X,I7,1X,I7,1X,I7,1X,I7,1X,F10.4,1X,F10.4)') &
         & schritt,EnergySum(1),EnergySum(2),EnergySum(3),EnergySum(4),numberParticles,notBound,outside,&
  ...
  subroutine evaluateBindingEnergy_Teis (teilchen, time)
    ...
    if (firstTime) then
       call writeFileDocu('BindingEnergy.dat', 'Includes binding energy of all nucleons, no Coulomb effects included')
       ...
       call writeFileDocu('BindingEnergyArr.dat', 'The distribution connected with the average in "BindingEnergy.dat"')
...
!             dE = teilchen(i,j)%perWeight*(/1.,EE,0./)+(/0.,0.,1./)
       ...
       if (EE_nucB(3)>0.) call AddHist(hBA,EE_nucB(2)/EE_nucB(1)-mN,EE_nucB(1)/EE_nucB(3))

    End do Loop_over_ensembles

    ! Printout the Energy per baryon:
    write(22,fmt='(F8.2,4ES14.6,4F10.3)') time, &
                                          E_all(2)/E_all(1), E_bar(2)/E_bar(1), E_nuc(2)/E_nuc(1), E_nucB(2)/E_nucB(1), &
                                          E_all(3)/nEns,     E_bar(3)/nEns,     E_nuc(3)/nEns,     E_nucB(3)/nEns
    close(22)
...
!   end subroutine checkTrajectories
  ...
  subroutine evaluateTimeStep(iflag,coll_num,delta_T_max,time,delta_T_new)
    ...
    real, parameter ::  kappa=1.   ! parameter which determines the slope of increasing time step when collision frequency drops
  ...
  ! subroutine CheckGridSize(teilchen,time,time_max,TheEventType,gridSpacing,gridSize)
  ! PURPOSE
  ...
  subroutine CheckGridSize(teilchen,time,time_max,TheEventType,gridSpacing,gridSize)
    ...
    if(initGridFlag) then
       ...
       if(TheEventType==1)   SystemSize = float(TheTarget%Mass + TheProjectile%Mass)
       if(TheEventType==300) SystemSize = float(TheTarget%Mass+1)
    ...
    Warning : if (.not.WarningFlag) then !do the check ONLY at once per subsequent run
       ...
       !--------------------------------------------------------------------
       !  if ( affected_events == nEns .and. .not.WarningFlag ) then
