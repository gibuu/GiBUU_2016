...
contains
  ...
  subroutine readinput
    ...
    NAMELIST /pn_medium/ pn_medium_switch, debug, density_dependent, form_factor
...
!    use init_pn_medium
    ...
    if (present(mu_extern)) then
       if (density_dependent) then
          proton_width_medium=proton_width_mdd(momentumLRF,pdens,ndens,mu_extern)
       else
          proton_width_medium=proton_width_mdd(momentumLRF,pdens,ndens,mu_extern)
       end if
    else
...
1002 format(4(e12.4,1x))
    ...
    momentumtmp=sqrt(Dot_product(momentumLRF(1:3),momentumLRF(1:3) ) )
    ...
    if (present(mu_extern)) then
       omegatmp=momentumLRF(0)-mu_extern+omega_f_width_field(0.5*rho_field(n_rho) )
    else
       omegatmp=momentumLRF(0)-mN !+omega_f_width_field(0.5*rho_field(n_rho) )
    end if
