...
contains
  ...
  subroutine readInput
    ...
    ! * extrapolateAbsP
    !*************************************************************************
    NAMELIST /baryonWidthMedium_tables/ debugFlag,minRes,maxRes,inMediumParameterset, &
      ...
      outerLoop: do i=1,2
         idLoop: do particleId=max(minRes,1),min(maxRes,nres+1)
            ...
            if((numSteps_absP.ne.numSteps_absP_in) &
               ...
               if(num_MonteCarlo_Points_in.ne.num_MonteCarlo_Points)  write(*,*) 'MC',num_MonteCarlo_Points,num_MonteCarlo_Points_in
               if(min_charge_loop_in.ne.get_min_charge_loop()) write(*,*) 'charge loop min',min_charge_loop_in,get_min_charge_loop()
               if(max_charge_loop_in.ne.get_max_charge_loop()) write(*,*) 'charge loop max',max_charge_loop_in,get_max_charge_loop()
