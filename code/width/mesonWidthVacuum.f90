...
contains
  ...
  real function semistableFinalState(mass,polemass,ID,L,partialWidth_pole)
    ...
    If (debug) then
       Print *, "Rho_AB_pole=", rho_ab_pole
       Print *, "Rho_AB_mass=", rho_ab_mass
    end if

    if (rho_ab_pole == 0.) then
       semistableFinalState = 0.
    else
       If (debug) Print *, partialWidth_pole,rho_ab_mass,rho_ab_pole
  ...
  real function rho_AB_Integrand (mu)
    ...
    If (idUnstable_copy==rho) then
       ...
       Print *, "Wrong Resonance as unstable Particle in rho_ab_integrand :",idUnstable_copy
       Print *, "This resonance is not yet implemented in subroutine 'semistableFinalState' :",idUnstable_copy
       Stop
    end if
    ...
    If (debug) then
      Print *, RelBW(mu,massUnStable,gamma),p_AB,srtS,BlattWeisskopf(p_AB*interactionRadius,L_copy)**2
      Print *,"rho_AB_Integrand=",rho_AB_Integrand
    end if
  ...
  real function omegaVacuum (s, r)
    ...
    use IDTable, only: pion, rho, omegaMeson
    use particleProperties, only: hadron
    use twoBodyTools, only: pCM
