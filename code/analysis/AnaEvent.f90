...
module AnaEvent
  ...
  ! SOURCE
  !
  logical, dimension(1:numStableParts),save :: particleIDs_flag=.true.
  ! PURPOSE
  ...
  PUBLIC :: event_sigma
  PUBLIC :: event_dSigma_dE,event_dSigma_dOmega,event_dSigma_dEcostheta,event_dSigma_dInvMass
...
!
!     Select Case(ID)
!     case(pion,eta,kaon,kaonBar,dMeson,dBar,nucleon,lambda,sigmaResonance,Xi,OmegaResonance, &
!        &  ds_plus,ds_minus)
...
!   end function event_getMultiplicities
  ...
  subroutine event_sigma(E,sigmaOut,newInit,runNumber,identifier)
    ...
    character(30), dimension(1:dimSigma,1:2) :: sigma_name ! Denotes the different output channels
    ...
    call get_init_namelist(outLepton_ID=outLeptonID, outLepton_charge=outLeptoncharge)
    ...
    idLoop: do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       chargeLoop: do charge=-2,2
          if(.not.validCharge_ID(particleIDs(i),charge)) cycle  ! Check that charge is valid
          channel=channel+1
          if (channel>dimSigma) then
             call TraceBack('index value channel > bounds')
          endif
          ...
          eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
             ! (2) Count contributions of any particles in the event with charge="charge"
    ...
    do numberParticles=1,4
       idLoop1: do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
          ...
          sigma_name(channel,1)=intTochar(channel)//': '//intTochar(numberParticles)//trim(name)
          ...
          eventLoop1: do j=lBound(E,dim=1),uBound(E,dim=1)
             ...
             ch_loop1: do charge=-2,2
                if(.not.validCharge_ID(particleIDs(i),charge)) cycle ! Check if charge is valid
                numInEventLoop: do num=1,numberParticles
                   ! if there are less than num particles in this event with that charge:
                   if( .not.event_GetParticle(E(j),particleIDs(i),charge,num,part)) &
                      & exit numInEventLoop
             ...
             if(foundIds.ne.numberParticles) then
                ...
                sigma(channel,1)=sigma(channel,1)+minval(perweight(1:numberParticles))
             end if
          end do eventLoop1
       end do idLoop1
    end do
...
!          if(E(j)%numberParticles(numStableMesons+1,0).ne.0) cycle
    ...
    idLoop_single: Do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       chargeLoop_single: do charge=-2,2
          if(validCharge_ID(particleIDs(i),charge)) then         ! Check if channel is valid
             channel=channel+1
             if (channel>dimSigma) then
                call TraceBack('index value channel > bounds')
             endif
             ...
             sigma_name(channel,2)=intTochar(channel+dimSigma)//': error '// trim(name)//' +X'
    ...
    ! - count each pi^0 seperately, e.g. if there are 2 they count twice!
    !*************************************************************
    ...
    eventLoop_Krusche3: do j=lBound(E,dim=1),uBound(E,dim=1)
       ...
       if( E(j)%numberParticles(numStableMesons+1,1).ge.1) then
          if( event_GetParticle(E(j),particleIDs(numStableMesons+1),1,1,part)) then
    ...
    if(initSigma) then
       ...
       ! and "multi" particle cross sections. Especially note the exclusive aspects.
       !
       !*******************************************************************
       ...
       write(12,formatOut)  sigma_name(:channel,1)(1:4),sigma_name(:channel,2)(1:4)
...
! For error evaluation we save the square of the change in the present run into sigma(:,2) :
!!$       do i=1,100
...
!!$       end do
       ...
       !write(12,formatOut) ' ',sigma(1,1),sigma(2:,1)/float(runNumber),sigma(:channel,2)
  ...
  subroutine event_dSigma_dOmega(E,dTheta,dPhi,string,runNumber,hists_theta,hists_phi,hists2D, &
    ...
    &   hists_theta,hists_phi
    type(histogram2D),optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists2D
       ...
       chargeLoop: do charge=-2,2
          ...
          if(makeInitHists) then
             ...
             call createHist(dsigma_dPhi, 'dSigma/dPhi('// trim(name) //&
                  & ') for single '//trim(name)// ' production' &
                  &  ,0. ,2.*pi,dPhi)
             call createHist2D(dsigma_dTheta_dPhi, 'dSigma/dTheta/dPhi('// trim(name) //&
                  & ') for single '//trim(name)// ' production' &
          ...
          if(sameFileName) then
             ...
             filename2D=trim(string)//'_dSigma_dTheta_dPhi_'// trim(name)&
                  & //'_charge_'//intTochar_pm(charge) //'.' //trim(intTochar(runNumber))//'.dat'
             filename_theta=trim(string)//'_dSigma_dTheta_'// trim(name)&
                  & //'_charge_'//intTochar_pm(charge)//'.' //trim(intTochar(runNumber))//'.dat'
             filename_phi=trim(string)//'_dSigma_dPhi_'// trim(name)&
                  & //'_charge_'//intTochar_pm(charge)//'.' //trim(intTochar(runNumber))//'.dat'
          ...
          eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
             if(exclusive_hadron) then
               ...
               ! now exclusive 1 baryon in final state, plus mesons
                  ...
                  &  .and.( sum(E(j)%numberParticles(i,:)).eq.1)) )   then
                   ...
                   !                    theta=degrees(theta)
                   !                    phi  =degrees(phi)
                   call addHist(dsigma_dTheta       , theta         ,part%perweight)
             ...
             if((E(j)%numberParticles(i,charge).eq.1).and.( sum(E(j)%numberParticles(i,:)).eq.1))&
                & then
                if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                   call get_phi_Theta(part%momentum(1:3),theta,phi)
                   !                    theta=degrees(theta)
          ...
          ! Output histograms
          If(optionals) then
             ! Make error calculation, therefore subtract the input from the present histogram
             ! to get the change
          ...
          if(makeoutput) then
             fak = 1.0
             If(.not.makeInitHists) fak=1./float(runNumber)! normalize to number of runs
  ...
  !*************************************************************************
  subroutine event_dSigma_dE(E,eMin,eMax,dE,string,runNumber,hists,initHists,makeOutputIn, &
    ...
    logical , intent(in), optional ::sameFileNameIn ! .true. = always print to same filenames.,
                                                    !.false. = different files for different runs
    logical , intent(in), optional ::makeOutputIn ! Flag to switch off output: .false. = no output
 ...
 !   character(100) :: fname
    real :: ekin
    type(histogram),optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists
    type(histogram),optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists1X
    type(histogram),optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists2X
    type(histogram),optional, intent(inout),dimension(1:numStableParts,-2:2) :: histsMulti
    logical, intent(in),optional :: initHists
    logical ::  makeInitHists
 ...
 ! then loop over individual charge states
       chargeLoop: do charge=-2,2
          ...
          if(makeInitHists) then
             ...
             call createHist(dsigma_dE_1X, 'dSigma/dEkin('// trim(name) //&
                  & ') for 1'//trim(name)// 'and X(including the same id but different charge)&
                  & production',eMin ,eMax,dE)
             call createHist(dsigma_dE_2X, 'dSigma/dEkin('// trim(name) //&
                  & ') for 2'//trim(name)// 'and X(including the same id but different charge)&
                  & production',eMin ,eMax,dE)
         ...
         if(i <= numStableMesons .and. exclusive_hadron) name1 = trim('excl'//name)
          if(sameFileName) then
             ...
             filename1X=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//  &
             &  intTochar_pm(charge) //'_1X.dat'
             filename2X=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//  &
             &  intTochar_pm(charge) //'_2X.dat'
             filenameMulti=trim(string)//'_dSigma_dEkin_'// trim(name)//'_charge_'//  &
             ...
             filename1X=trim(string)//'_dSigma_dEkin_'// trim(name)&
                  & //'_charge_'//intTochar_pm(charge) //'.' //trim(intTochar(runNumber))// &
                  &  '_1X.dat'
      ...
      ! Find out whether there are any particles in the event 'j' with charge="charge"
      ! and ID="particleIDs(i)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
             ...
             !  in the event)
             !Find out whether there are any particles in the event with charge="charge"
             ! and ID="particleIDs(i):
             ...
             if(E(j)%numberParticles(i,charge).eq.2) then
                do particle_number=1,2
                   if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                      ekin=part%momentum(0)-part%mass
             ...
             ! and ID="particleIDs(i):
             if(E(j)%numberParticles(i,charge).ge.1) then
                !write(*,'(3(A,I5))') 'i=',i, '   charge=',charge,  'numberParticles=', &
                ! & E(j)%numberParticles(i,charge)
                do particle_number=1,E(j)%numberParticles(i,charge)
                   if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                      ekin=part%momentum(0)-part%mass
                      !write(*,'(A,I5,2(A,g15.5))') 'particle_number=', particle_number, ' &
  ...
  ! charge states.
  ! Theta is the angle of the outgoing particle with respect to the neutrino beam direction
  !
  ...
  !   and "STRING" by the input string.
  !*************************************************************************
  subroutine event_dSigma_dEcostheta(E,EcosthetaMin,EcosthetaMax,dEcostheta,string,runNumber, &
     & hists,hists_c, hists_MULTI, hists_c_MULTI,  initHists,  makeOutputIn, sameFileNameIn)
    use particleDefinition
    use IdTable, only: isHadron
    ...
    ! Optional
    type(histogram)  ,optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists
    type(histogram)  ,optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists_MULTI
    type(histogram)  ,optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists_c
    type(histogram)  ,optional, intent(inout),dimension(1:numStableParts,-2:2) :: hists_c_MULTI
    logical, intent(in),optional :: initHists
    ! Input
    type(tAnaEvent) , intent(in), dimension(:) :: E ! List of events
    ! Local
    type(particle) :: part
    type(histogram) ::    dsigma_dEcostheta, dsigma_dcostheta, dsigma_dEcostheta_MULTI,&
...
!    character(100) :: fname
    ...
    idLoop: Do i=lBound(particleIDs,dim=1),uBound(particleIDs,dim=1)
       ...
       chargeLoop: do charge=-2,2
          if(validCharge_ID(particleIDs(i),charge)) then
             ...
             if(makeInitHists) then
             ! Initialize histogram
                call createHist(dsigma_dEcostheta, 'dSigma/dEcostheta('// trim(name1) //&
                     & ') for single '//trim(name1)// ' production' &
                     &  ,EcosthetaMin,EcosthetaMax,dEcostheta)
                call createHist(dsigma_dcostheta, 'dSigma/dcostheta('// trim(name1) //&
                     & ') for single '//trim(name1)// ' production' &
                     &  ,-1.0,1.0,0.02)
                call createHist(dsigma_dEcostheta_MULTI, 'dSigma/dEcostheta('// trim(name) //&
                     & ') for multiple '//trim(name)// ' production' &
                     &  ,EcosthetaMin,EcosthetaMax,dEcostheta)
                call createHist(dsigma_dcostheta_MULTI, 'dSigma/dcostheta('// trim(name) //&
                     & ') for multiple '//trim(name)// ' production' &
                     &  ,-1.0,1.0,0.02)
             else
             ...
             if(sameFileName) then
                filename_Ecostheta=trim(string)//'_dSigma_dEcostheta_'// trim(name1)&
                     & //'_charge_'//intTochar_pm(charge) //'.dat'
                filename_costheta=trim(string)//'_dSigma_dcostheta_'// trim(name1)&
                     & //'_charge_'//intTochar_pm(charge) //'.dat'
                filename_Ecostheta_MULTI=trim(string)//'_dSigma_dEcostheta_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'_MULTI.dat'
                filename_costheta_MULTI=trim(string)//'_dSigma_dcostheta_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'_MULTI.dat'
             else
                filename_Ecostheta=trim(string)//'_dSigma_dEcostheta_'// trim(name1)&
                     & //'_charge_'//intTochar_pm(charge)//'.' //trim(intTochar(runNumber))//'.dat'
    ...
    excl: if(exclusive_hadron) then
         ...
         getpart:  if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                        If(dot_product(part%momentum(1:3),part%momentum(1:3)).gt.0) then
                           costheta=part%momentum(3)/sqrt(dot_product(part%momentum(1:3), &
                              &  part%momentum(1:3)))
                        else
                           costheta=0.
                        end if
             ...
             If(optionals) then
                ...
                If(.not.makeInitHists) then
                   ...
                   call makeError_hist(hists_MULTI(i,charge),dSigma_dEcostheta_MULTI)
                   call makeError_hist(hists_c_MULTI(i,charge),dSigma_dcostheta_MULTI)
                else
             ...
             if(makeoutput) then
                ...
                If(makeInitHists) then
                   ...
                   ! normalize to number of runs
                   call WriteHist(dsigma_dEcostheta_MULTI,12,0.,1./float(runNumber))
                end if
                close(12)
                ...
                If(makeInitHists) then
                   ...
                   ! normalize to number of runs
                   call WriteHist(dsigma_dcostheta_MULTI,12,0.,1./float(runNumber))
                end if
                close(12)
  ...
  !*************************************************************************
  subroutine event_dSigma_dInvMass(E,string,runNumber, &
       & dW_Npi,Wmin_Npi,Wmax_Npi,histsW_nucleon_pion, dW_mupi,Wmin_mupi,Wmax_mupi, &
       & histsW_muon_pion,dW_muN,Wmin_muN,Wmax_muN,histsW_muon_nucleon, initHists,makeOutputIn, &
       & sameFileNameIn)
    ...
    character(*), intent(in) :: string ! used as prefix for all output files
    integer, intent(in) :: runNumber ! number of the run
    type(histogram),optional, intent(inout),dimension(-1:1) :: histsW_nucleon_pion, &
    ...
    pionChargeLoop: do charge=-1,1
       ...
       if(makeInitHists) then
          ...
          call createHist(dsigma_dW_muon_pion, 'dSigma/dW_muon_pion for 1 pion production', &
               &  Wmin_mupi, Wmax_mupi, dW_mupi)
          call createHist(dsigma_dW_muon_nucleon, 'dSigma/dW_muon_nucleon for 1 pion production', &
               &  Wmin_muN, Wmax_muN, dW_muN)

          !          write(*,'(A,I5)') 'pion charge=', charge
          !          write(*,'(9(A,g10.3))') 'Wmin_Npi=',Wmin_Npi, '   Wmax_Npi=',Wmax_Npi,&
          !          &  '  dW_Npi=',dW_Npi, '   Wmin_Npi=',Wmin_mupi,'   Wmax_Npi=',Wmax_mupi,  '&
          !          &  dW_Npi=',dW_mupi,'   Wmin_Npi=',Wmin_muN, '   Wmax_Npi=',Wmax_muN, &
          !          & '  dW_Npi=',dW_muN

       else
          dsigma_dW=histsW_nucleon_pion(charge)
          dsigma_dW_muon_pion=histsW_muon_pion(charge)
          dsigma_dW_muon_nucleon=histsW_muon_nucleon(charge)  ! this is the charge of pion,
                                                              ! not nucleon
       end if
       ...
       !Find out whether there are pairs (nucleon-pion, muon-pion, muon-nucleon) in the event:
       eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
          ! search for event with 1 pion  and 1 nucleon (and 1 muon automatically)
          if((          E(j)%numberParticles(1,charge).eq.1) .and. &  ! in numberParticles pion=1
             ...
             ! ---------------------------------
             if( event_GetParticle(E(j),101,charge,1,part_pion)) then  ! ID of pion is 101

                !write(*,'(A,4g12.5,A,g12.5)') 'Found pion with 4-momentum', part_pion%momentum, &
                !                         & '  and abs4(4-momentum) is', abs4(part_pion%momentum)
                !write(*,'(A,g12.5)') 'pion perweight=', part_pion%perweight


                nucleon_charge=-1
                do k=0,1
                   if(  event_GetParticle(E(j),1,k,1,part_nucleon) ) then ! ID of nucleon is 1
                      nucleon_charge=k
                      !write(*,'(A,4g12.5,A,g12.5)') 'Found nucleon with 4-momentum',
                      !              & part_nucleon%momentum, &
                      !              & '  and abs4(4-momentum) is', abs4(part_nucleon%momentum), &
                      !              & '  and charge is', nucleon_charge
                      !         write(*,'(A,g12.5)') 'nucleon perweight=', part_nucleon%perweight
                      exit
                   end if
                end do

                if(nucleon_charge.ge.0 .and. event_GetParticle(E(j),outLeptonID,outLeptonCharge, &
                    &  1,part_muon) ) then
             ...
             if (W2.gt.0 .and. W2_muon_pion.gt.0 .and. W2_muon_nucleon.gt.0) then
                call addHist(dsigma_dW              , sqrt(W2)              ,part_muon%perweight)
                call addHist(dsigma_dW_muon_pion    , sqrt(W2_muon_pion)    ,part_muon%perweight)
                call addHist(dsigma_dW_muon_nucleon , sqrt(W2_muon_nucleon) ,part_muon%perweight)
                !write(*,'(4(A,g12.5))') 'W2=',  W2, '   W2_muon_pion=', W2_muon_pion, &
          ...
          ! end of search for event with 1 pion  and 1 nucleon (and 1 muon automatically)
       ...
       if(present(histsW_nucleon_pion) .and. present(histsW_muon_pion)  &
          ...
          if(.not.makeInitHists) then
             ...
             call makeError_hist(histsW_muon_nucleon(charge),dsigma_dW_muon_nucleon)
             ! Save the list
       ...
       if(makeOutput) then
          ...
          If(makeInitHists) then
             write(12,*)'# Notice ! The charge in the name of this file is pion charge'
  ...
  ! NAME
  ! subroutine event_dSigma_dLeptonVariables(E, eMin, eMax, dE, string,
  ! runNumber, hist_Enu, hist_E, hist_cos, hist_Q2,
  ...
  ! * type(histogram), intent(inout),
  !   dimension(lBound(particleIDs,dim=1):uBound(particleIDs,dim=1),-2:2) ::
  !   hist_Enu, hist_E,  hist_cos, hist_Q2
  ! * logical, intent(in) :: initHists
  ! * logical :: makeOutputIn -- Flag to switch off output (.false.=no output)
  ! * logical, intent(in) :: sameFileNameIn  --
  !   .true. = always print to same filenames,
  ...
  !   and "STRING" by the input string.
  !*************************************************************************
  subroutine event_dSigma_dLeptonVariables(E,eMin,eMax,dE,string,runNumber,hist_Enu,hist_E, &
     &  hist_cos,hist_Q2,initHists, makeOutputIn, sameFileNameIn, specificEvent)
    ...
    if(makeInitHists) then
       call createHist(dsigma_dEnu, 'dSigma/dEnu( incoming neutrino ) for events'//trim(name) , &
          &  eMin ,eMax,dE)
       call createHist(dsigma_dE,   'dSigma/dEkin( outgoing lepton ) for events'//trim(name) , &
          &  eMin ,eMax,dE)
       call createHist(dsigma_dcos, 'dSigma/dcostheta( outgoing  lepton ) for events'//trim(name),&
          &  -1.0, 1.0, 0.02)
       ! Q2bin and Q2max are taken numerically the same as those for energy
       call createHist(dsigma_dQ2,  'dSigma/dQ^2( outgoing  lepton ) for events'//trim(name), &
          &  0.0, eMax, dE)
    else
    ...
    eventLoop: do j=lBound(E,dim=1),uBound(E,dim=1)
       ...
       if (pass) then
          ...
          If( -abs4Sq(boson_mom) .gt. 0) then
             ...
             write(*,*) 'In event_dSigma_dLeptonVariables strange Q2=', -abs4Sq(boson_mom)
             Q2=-1.
          end if
...
!   end subroutine event_Print
  ...
  subroutine event_add(L, V)
    ...
    if(.not.V%antiparticle) then
       Select Case(V%ID)
 !       case(particleIDs(1):)
        case(pion,eta,kaon,kaonBar,dMeson,dBar,ds_plus,ds_minus,nucleon,lambda,sigmaResonance,Xi,OmegaResonance)
 !
 ...
 !
       ...
       case Default
          L%numberParticles(numStableParts+1,V%charge)=L%numberParticles(numStableParts+1, &
             &  V%charge)+1
       end select
    else if(isMeson(V%ID)) then
       write(*,*) 'Error in event_add: We expect no ANTI-Mesons!!! Convert these first Stop!!'
  ...
  !
  ! PURPOSE
  ! Asume that a%yval(:,1) contains the following : sum over all
  ! runs(xSection of run).
  ! And asume that a%yval(:,3) contains the following : sum over all
    ...
    if(numRuns.le.1) then
       ...
       do i=lbound(a%yval,dim=1),ubound(a%yval,dim=1)
          ...
          if(a%yval(i,3).lt.(a%yval(i,1)**2)/float(numRuns)) then
             write(*,'(A,2E20.10)') 'Error in setError_hist',a%yval(i,3),(a%yval(i,1)**2)&
                  & /float(numRuns)
             ...
             dummy=float(numRuns)*sqrt(max(1./float(numRuns-1)/float(numRuns)*(a%yval(i,3) &
                 & -(a%yval(i,1)**2)/float(numRuns)),0.))
             a%yval(i,3)=dummy
          end if
       end do
    end if
...
!
!     do i=1,12
!        write(*,'(i3,":",5i5," !",i5)') i,E%numberparticles(i,:),sum(E%numberparticles(i,:))
!     enddo
!     write(*,'("---:-------------------------------")')
!     write(*,*)
!
!   end subroutine event_DumpNumbers
  ...
  subroutine event_pairPhotons(L,fileName,dsigma_dm_threeGammas,Hist_massPhotons)
    ...
    character(100) :: fileName_data,fileName_statistics,filename_dsdm  ! file name to write to
    type(tParticleListNode),Pointer       :: pNode
    character(20),parameter :: out='(2I5,I7,8E14.6)'
    ...
    if (.not.allocated(Hist_massPhotons%yVal)) then
       call CreateHist2D(Hist_massPhotons, 'Mass(2 gamma) vs. Mass(3 gamma)',(/0.,0./), &
          & (/1.0,1.0/),(/0.03,0.03/),.true.)
       do i=lbound(dsigma_dm_threeGammas,dim=1),ubound(dsigma_dm_threeGammas,dim=1)
          call CreateHist  (dsigma_dm_threeGammas(i), 'dsigma/dm (3 Gammas)', 0.,1.,0.01)
       end do
    ...
    write(48,'(A)') '# This statistic gives the number of produced photons in each event'
    write(48,'(A)') '# Note: Events with no final state particles, i.e. Pauli blocked events, ' &
    ...
    eventLoop:  do i=lbound(L,dim=1),ubound(L,dim=1)
       ...
       particleLoop: do
          ...
          if (pNode%V%ID.eq.photon) then
             if(indexPhoton.eq.1) perweight=0 ! Initialize for the case that there is a photon
             if(indexPhoton.le.ubound(momentum,dim=1)) then
                ...
                write(*,*) 'WARNING: More than ', ubound(momentum,dim=1), ' photons in event #', i
             end if
          end if
          pNode => pNode%next
       end do particleLoop
       ...
       if(atLeastOneParticle) then
          ...
          if(numPhotons.ne.0) then
             ...
             list_numPhotons(numPhotons,2) =list_numPhotons(numPhotons,2)+default_perweight
          end if
       end if
          ...
          do massLoop_i=1,numPhotons-2
             do massLoop_j=massLoop_i+1,numPhotons-1
                do massLoop_k=massLoop_j+1,numPhotons
                   !write(*,*) massLoop_i,massLoop_j,massLoop_k,perweight((/massLoop_i, &
                   ! massLoop_j, massLoop_k/))
                   ! Evaluate the mass of the triple and the masses of all possible pairs:
                   call getMasses( momentum((/massLoop_i, massLoop_j, massLoop_k/),0:3), &
                      & mass_all,mass_pairs)
                   ! Protocol result
                   call AddHist(dsigma_dm_threegammas(0),mass_all,minval(perweight &
                   ...
                   do index=1,3
                      call AddHist2D(hist_massPhotons, (/mass_all,mass_pairs(index) /),&
                           & minval(perweight((/massLoop_i, massLoop_j, massLoop_k/)))/numRuns)
                      if(abs(minval(perweight((/massLoop_i, massLoop_j, massLoop_k/)))) &
                         & .lt.0.0001) then
  ...
  !
  ! Additionally info "prod_id  Enu  Enu_restored  nu  Ehad  perweight" for all events
  ...
  ! * type(histogram),dimension(0:5) ::  Enurestored_versusEnu, dSigdEnu, dSigdEnurestored
  !   where  0=all events, 1=QE, 2=Delta, 3=highRES, 4=bgr, 5=DIS
  !*************************************************************************
  subroutine event_hadronicEnergy(L, numin, numax, nubin, Ehad_versusNu, dSigdNu, dSigdEhad, &
                                           & Enumin, Enumax, Enubin, Enurestored_versusEnu,&
    ...
    real, intent(in) :: numin, numax, nubin ! boundaries for the hystograms
                                            ! in the namelist "nl_calorimetric_analysis"
                                            ! in neutrinoAnalysis.f90
    real, intent(in) :: Enumin, Enumax, Enubin ! boundaries for the hystograms
                                               ! in the namelist "nl_calorimetric_analysis"
    type(histogram2D),dimension(0:max_Hist), intent(inout) :: Ehad_versusNu, Enurestored_versusEnu
    type(histogram), dimension(0:max_Hist),intent(inout) :: dSigdNu, dSigdEhad, dSigdEnu, &
       &  dSigdEnurestored
    ...
    if (.not.allocated(Ehad_versusNu(0)%yVal)) then
       do iHist=0, max_Hist ! 0=all 1=QE 2=Delta 3=highRES 4=1pibgr 5=DIS 6=2p2hQE 7=2p2hDelta &
                            ! 8=2pibgr
          if (.not.includeHist(iHist)) cycle
          call CreateHist2D(Ehad_versusNu(iHist), 'Nu -- Ehad plane',(/numin,numin/),  &
             &  (/numax,(numax+1.)/),(/nubin,nubin/),.true.)
          call CreateHist  (dSigdNu(iHist), 'dSigdNu', numin,numax,nubin)
          call CreateHist  (dSigdEhad(iHist), 'dSigdEhad', numin,(numax+1.),nubin)
          call CreateHist2D(Enurestored_versusEnu(iHist), 'Enu -- Enu_restored plane', &
               & (/Enumin,Enumin/),(/Enumax,Enumax/),(/Enubin,Enubin/),.true.)
          call CreateHist  (dSigdEnu(iHist), 'dSig/dEnu', Enumin,Enumax,Enubin)
          call CreateHist  (dSigdEnurestored(iHist), 'dSig/dEnu_restored', Enumin,Enumax,Enubin)
       end do
       numRuns = num_Runs_sameEnergy * num_Energies
    end if

    open(60,file="calorim_events_analysis.dat")
    write(60,*) '# 1:prod_id 2:enu 3:enu-restored 4:nu=energy-transferre 5:Ehad 6:perweight'
    close(60)
    ...
    eventLoop:  do i=lbound(L,dim=1),ubound(L,dim=1) ! "i" - first event
       ...
       if (debug) write(*,'(2(A,I5),A,g14.5)') 'first_event=',i, '      prod_ID=',prod_ID, &
          &  '     nu=', boson_mom(0)
       Ehad=0.
       ...
       particleLoop: do
          ...
          if( isBaryon(pNode%V%ID) ) then
             ! for baryons add  energy-nucleon mass, for antybaryons energy+nucleon mass
             if (pNode%V%antiparticle) then
                Ehad=Ehad+pNode%V%momentum(0)+mN !antibaryon energy
                                                 ! + compensation for an accompaning baryon
             else
       ...
       if (debug) write(*,*) ' ------------------------------------ '
       write(60,'(I5,5g14.5)') prod_id, boson_mom(0)+lep_mom(0), Ehad+lep_mom(0), boson_mom(0), &
          &  Ehad, perweight
       ...
       call AddHist2D( Ehad_versusNu(0),  Ehad_versusNu(iHist), (/boson_mom(0), Ehad/), perweight)
       ...
       call AddHist( dSigdEnu(0), dSigdEnu(iHist),  boson_mom(0)+lep_mom(0),  perweight)
       call AddHist( dSigdEnurestored(0), dSigdEnurestored(iHist), Ehad+lep_mom(0),    perweight)
    ...
    do iHist=0,max_Hist
       ...
       call WriteHist2D_Gnuplot(Enurestored_versusEnu(iHist),&
            & mul=1.0/numRuns,&
            & file=trim(filename_Enurestored_versusEnu)//trim(intToChar(iHist))//'.dat',&
            & dump=.false.)
       ...
       call WriteHist(dSigdEnurestored(iHist), &
            & mul=1.0/numRuns,&
            & file=trim(filename_dSigdEnurestored)//trim(intToChar(iHist))//'.dat',&
            & dump=.false.)
    end do
  ...
  subroutine SpecificEvent_Name(specificEvent,name)
    ...
    case (19)
       name = 'excl_piminus'
    case default
       write(*,*) 'In SpecificEvent_Name WRONG specificEvent=', specificEvent, 'STOP'
       stop
    end select
