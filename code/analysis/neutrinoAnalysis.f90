...
module neutrinoAnalysis
 ...
 !***************************************************************************
  ...
  ! Maximal kinetic energy for dsigma/dEkin
  !***************************************************************************
  ...
  !***************************************************************************
  !****g* neutrinoAnalysis/EnergyDetectionThreshold_lepton
  ! SOURCE
  !
  ...
  ! PURPOSE
  ! do analysis for specific final states: specificEvent=4, 1 pi0 X nucleons,
  ! pi0 (1pi0 MiniBooNE)
  !
  ! values can be changed in the namelist nl_specificEvent
  !***************************************************************************
  ...
  ! Such events (very rare at DUNE energies) could be counted as exclusive single-meson
  ! cross section. This could be cured by extending the list of stable mesons
  ...
  type(histogram),save, dimension(1:max_SpeEvent,0:max_Hist) :: dEnu_hist, dElepton_hist, &
      & dcoslepton_hist, dQ2lepton_hist



  type(histogram),save,dimension(1:numStableParts,-2:2) :: dTheta_hists,dPhi_hists, &
       & dTheta_hists_QE,dPhi_hists_QE, &
       & dTheta_hists_Delta,dPhi_hists_Delta,dTheta_hists_highRes,dPhi_hists_highRes, &
       & dTheta_hists_gen0,dPhi_hists_gen0, &
  ...
  type(histogram2D),save,dimension(1:numStableParts,-2:2) :: dOmega_hists, &
       ...
       dOmega_hists_highRes,dOmega_hists_gen0,dOmega_hists_1piBG,dOmega_hists_2piBG
  ...
  type(histogram),save,dimension(1:numStableParts,-2:2) :: dEcostheta_hists_MULTI, &
       ...
       & dEcostheta_hists_1piBG_MULTI,dEcostheta_hists_2piBG_MULTI,dEcostheta_hists_DIS_MULTI
  type(histogram),save,dimension(1:numStableParts,-2:2) :: dcostheta_hists,dcostheta_hists_QE, &
...
contains
  ...
  subroutine readinput
    ...
    !*************************************************************************
    NAMELIST /neutrinoAnalysis/ detailed_diff_output, include_W_dist, &
                                kineticEnergyDetectionThreshold_nucleon, &
                                kineticEnergyDetectionThreshold_chargedpion, &
                                kineticEnergyDetectionThreshold_neutralpion, &
                                inclusiveAnalysis, Fissum_analysis, ZeroPion_analysis, &
                                calorimetric_analysis, &
    ...
    NAMELIST /detailed_diff/ ekinMin,ekinMax, dEkin, &
         &  forpion, foreta, forkaon, forkaonBar, forDmeson, forDbar, forDs_plus, forDs_minus, &
 ...
 ! ie. exactly 1 meson with fixed charged and no other mesons of any kind
  ...
  subroutine neutrino_Analyze(Particles,finalFlag,num_runs_sameEnergy)
    ...
    use initNeutrino, only : includeQE, includeDELTA, includeRES, include1pi, includeDIS, &
         & include2p2hQE, include2p2hDelta, include2pi
    use minkowski, only: abs4, abs4Sq
    use vector, only : absVec
    use constants, only : pi
    use MultiplicityAnalysis, only: Multiplicity_Reset, Multiplicity_AddEvent, Multiplicity_Write
    ...
    type(tAnaEvent), Allocatable, dimension(:) :: events,events_QE,events_Delta,events_highRES, &
         & events_1piBG,events_DIS,events_2p2h, events_2piBG
    type(tAnaEvent), Allocatable, dimension(:) :: events_gen0,events_gen1,events_gen2, &
    ...
    ! In these hists we save the information which the "AnaEvent" subroutines are returning
    real, dimension(1:dimSigma,1:2),save :: sigma, sigma_QE,sigma_Delta,sigma_highRES,sigma_1piBG, &
       & sigma_2piBG,sigma_DIS,sigma_2p2h
    real, dimension(1:dimSigma,1:2),save :: sigma_gen0,sigma_gen1,sigma_gen2,sigma_gen3ormore

    ! In these hists we save the information which the "ZeroPionAnalysis" subroutines are returning:
    !  in particular channels  with "k" protons and "n" neutrons
    real, dimension(1:dimSigma,1:2),save :: sigma_0pions, sigma_QE_0pions,sigma_Delta_0pions, &
    ...
    real,dimension(-1:1) :: tsigmanucleon=0.          ! in DIS antiproton are also possible
                                                      ! in final states
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       write(10,*)'# Total xsec for outgoing pions and nucleons: each pion/nucleon contribute&
          & to the xsec'
       write(10,*)'# 1: raiseVariable  2: sigma(pi-)    3:pi0    4:pi+   5:antiproton &
          &  6:neutron, 7:proton'
       close(10)
       ...
       write(10,*)'# order of entries: 1:raise variable  2:ekin, 3:sig pi-, 4:sig pi0, 5:sig pi+,&
          & 6:sig neut, 7:sig prot'
       close(10)
       ...
       write(10,*) '#1:raiseFlagVariable, #2:p_tot, #3:p_out, #4:p_out_gen0, #5:p_out_gen1, &
       ...
       if(ZeroPion_analysis) then
          ...
          call WriteHeader10(include2p2hQE .or. include2p2hDelta,'neutrino_0pions_2p2h.dat')
       end if
       ...
       if(reconstruct_neutrino_energy .and. specificEvent_Analysis) then
          ...
          do m=1, max_speEvent
             ...
             do iHist=0, max_Hist
                ...
                ! Q2 reconstruction
                call CreateHist(H_Q2_real(m,iHist), 'true Q2 for a specific event',0.,maxQ2, &
                   & binsizeQ2)
                call CreateHist(H_Q2_rec(m,iHist),  'reconstructed Q2 for a specific event',0., &
                   & maxQ2,binsizeQ2)
                ...
                call CreateHist(H_enu_rec(m,iHist),  'reconstructed Enu for a specific event', &
                   & 0.,maxEnu,binsizeEnu)
                ...
                if (OSC(nuEXP)) then
                   ...
                   call CreateHist(Oscmue_enu_rec(m,iHist), &
                        'nu_e appearence versus reconstructed energy',0.,maxEnu,binsizeEnu)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !total (tsigma) and kinetic energy differential (tksigma) cross section for pions and nucleons
...
! Fissum analysis for O16(e,e'p) data, Phys.Rev.C70:034606,2004
             ...
             if(Fissum_analysis) then
                ...
                Emiss_Fissum(ntk)=Particles(i,j)%perweight/dEkin/0.02+Emiss_Fissum(ntk)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    write(10,'(10g13.5)') raiseFlagVariable, sum_tsigmapion/real(numberofcalls), &
    ...
    do i=0,200
       write(11,'(10g13.5)')raiseFlagVariable,  &
            ...
            sum_tksigmapion(0,i)/real(numberofcalls),sum_tksigmapion(1,i)/real(numberofcalls), &
            sum_tksigmanucleon(0,i)/real(numberofcalls),sum_tksigmanucleon(1,i)/real(numberofcalls)
    end do
    close(11)
    ...
    write(12,'(30g13.5)') raiseFlagVariable, &
         sum_numberofprotons_tot/numberofcalls, sum_numberofprotons_out/numberofcalls, &
         sum_numberofnucleons_out(1,:)/numberofcalls, &
         sum_numberofneutrons_tot/numberofcalls, sum_numberofneutrons_out/numberofcalls, &
         sum_numberofnucleons_out(0,:)/numberofcalls, &
         sum_numberofANTIprotons_tot/numberofcalls, sum_numberofANTIprotons_out/numberofcalls, &
         sum_numberofnucleons_out(-1,:)/numberofcalls
    close(12)
...
! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call WriteHist(hNucleonVacuumMass,file="NucleonVacuumMass.dat",mul = 1.0/numberofcalls)
...
!************************************************************************
  !now event analyses with more detailed information on multiplicities, generations
...
! (1) Setting up the particles into the events
 ...
 !
 ! First get 'in' and 'out' lepton momenta and momentum transfer for each event; the loop runs
 ! over all events
 !
...
! (2) Use the list "events" to evaluate cross sections
    ...
    call event_sigma(events_1piBG,sigma_1piBG,initHists,numberOfCalls,identifier=raiseFlagVariable)
    call event_sigma(events_2piBG,sigma_2piBG,initHists,numberOfCalls,identifier=raiseFlagVariable)
    call event_sigma(events_DIS,sigma_DIS,initHists,numberOfCalls,identifier=raiseFlagVariable)
    call event_sigma(events_2p2h,sigma_2p2h,initHists,numberOfCalls,identifier=raiseFlagVariable)
    ...
    if (specificEvent_Analysis) then
        do m=1, max_speEvent
          ...
          do iHist=0, 0
            ...
            call event_dSigma_dLeptonVariables(events,EkinMin,EkinMax,dEkin, &
                  & 'diff_'//trim(intToChar(iHist)),numberOfCalls, &
                  & dEnu_hist(m,iHist), dElepton_hist(m,iHist), dcoslepton_hist(m,iHist), &
                  & dQ2lepton_hist(m,iHist), &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  ! PURPOSE
  ! Return whether particle is below threshold
  !***************************************************************************
  logical function IsBelowThreshold(part)
    use particleDefinition
    ...
    !kinetic energy threshold for nucleons
    if(part%id.eq.1.and.(part%momentum(0)-part%mass).lt.kineticEnergyDetectionThreshold_nucleon) &
         IsBelowThreshold=.true.

    !kinetic energy threshold for charged pions
    if(part%id.eq.101.and.part%charge.ne.0.and.(part%momentum(0)-part%mass).lt. &
         & kineticEnergyDetectionThreshold_chargedpion) IsBelowThreshold=.true.

    !kinetic energy threshold for neutral pions
    if(part%id.eq.101.and.part%charge.eq.0.and.(part%momentum(0)-part%mass).lt. &
         & kineticEnergyDetectionThreshold_neutralpion) IsBelowThreshold=.true.
