...
!
! NOTES
! ZeroPionAnalysis = the code of AnaEvent.f90  is just adapted for additional final states
! It was created as separate module because there were no vacant colums for extra final states left in sigma.dat
! The output is in files sigma_0pions.dat, neutrino_0pions.dat, neutrino_0pions_QE.dat and so on
!
!***************************************************************************
module ZeroPionAnalysis
...
contains
  ...
  subroutine event_sigma_0pions(E,sigmaOut,newInit,runNumber,identifier)
    ...
    character(31), dimension(1:dimSigma,1:2) :: sigma_name  ! Denotes the different channels, used for output
    ...
    do j=lBound(E,dim=1),uBound(E,dim=1)
       if(sum(E(j)%numberParticles(1,-1:1)).eq.0) then
          perweight=0.
          if(event_GetParticle(E(j),outLeptonID,outLeptonCharge,1,part))  perweight=part%perweight
    ...
    !*************************************************************
    ! Special channels: no pions and "k" protons and "i" neutrons and everything else
    ! = 0 pions  "k" protons  "i" neutrons  ! for LAr chambers
    !*************************************************************
    ...
    channel=channel+1
    sigma_name(channel,1)=intTochar(channel)//': 0pi '//intTochar(k)//'p '//intTochar(i)//'n'
    sigma_name(channel,2)=intTochar(channel+dimSigma)//': error 0pi '//intTochar(k)//'p '//intTochar(i)//'n'
      ...
      if(sum(E(j)%numberParticles(1,-1:1)).eq.0) then ! no pions
          if(E(j)%numberParticles(7,1).eq.k) then ! "k" protons
              if(E(j)%numberParticles(7,0).eq.i) then ! "i" neutrons
                 perweight=0.
                 if(event_GetParticle(E(j),outLeptonID,outLeptonCharge,1,part))  perweight=part%perweight
                 !write(*,*) 'found event number j=',j, '  with  k=',k, 'protons and i=', i, '  neutrons', &
    ...
    channel=channel+1
    sigma_name(channel,1)=intTochar(channel)//': '//intTochar(k)//'pi- '//intTochar(i)//'pi0 '//intTochar(l)//'pi+'
    sigma_name(channel,2)=intTochar(channel+dimSigma)//': error '//intTochar(k)//'pi- '//intTochar(i)//'pi0 '//intTochar(l)//'pi+'
              ...
              if(E(j)%numberParticles(1,-1).eq.k) then ! "k" pi-
                  if(E(j)%numberParticles(1,0).eq.i) then ! "i" pi0
                     if(E(j)%numberParticles(1,1).eq.l) then ! "l" pi+
                        ...
                        if(associated(E(j)%particleList%first))  perweight=E(j)%particleList%first%V%perweight
                        !write(*,'(5(A,I5),A,f12.4)') 'found event number j=',j, '  with  k=',k, ' pi- and i=', i, ' pi0  and l=',l,  &
                        !          & 'pi+ ;     nucleon charge is', charge, '   perweight=', perweight(1)
                        sigma(channel,1)=sigma(channel,1)+perweight
                        !write (*,*) 'channel=', channel, '  sigma=', sigma(channel,1)
                     end if  ! pi+
                  end if  ! pi0
               end if  ! pi-
          end if ! pions
    ...
    channel=channel+1
    sigma_name(channel,1)=intTochar(channel)//': 1'//trim(PartName(101,k,.false.))//' X'
    sigma_name(channel,2)=intTochar(channel+dimSigma)//': error 1'//trim(PartName(101,k,.false.))//' X'


    do j=lBound(E,dim=1),uBound(E,dim=1)
       if(E(j)%numberParticles(1,k).eq.1) then ! "k" is the charge of the pion
          perweight=0.
          if(event_GetParticle(E(j),1,k,1,part)) perweight=part%perweight ! read perweight from outgoing pion
          sigma(channel,1)=sigma(channel,1)+perweight
       end if
    end do
    end do ! k
    ...
    !*************************************************************
    ! Special channels: no pions and "k" protons and any number of  neutrons and everything else
    ! = 0 pions  "k" protons  "X" neutrons ! for LAr chambers
    !*************************************************************
    ...
    sigma_name(channel,2)=intTochar(channel+dimSigma)//': error 0pi '//intTochar(k)//'p X'
    ...
    do j=lBound(E,dim=1),uBound(E,dim=1)
       if(sum(E(j)%numberParticles(1,-1:1)).eq.0) then ! no pions
          if(E(j)%numberParticles(7,1).eq.k) then ! "k" protons
             perweight=0.
             if(event_GetParticle(E(j),outLeptonID,outLeptonCharge,1,part))  perweight=part%perweight
             !write(*,*) 'found event number j=',j, '  with  k=',k, 'protons X neutrons', &
    ...
    if(initSigma) then
       ...
       write(14,formatOut)  sigma_name(:channel,1)(1:4),sigma_name(:channel,2)(1:4)
    ...
    if(present(sigmaOut).and.present(runNumber).and.present(newInit)) then
       ...
       do i=1,100
          if(runNumber.gt.1) then
             ...
             sigma(i,2)=sqrt(max(((sigma(i,2)-sigma(i,1)**2/float(runNumber))/float(runNumber-1)/float(runNumber)),0.))
          else
             sigma(i,2)=0.
          end if
       end do
       ...
       !write(14,formatOut) ' ',sigma(1,1),sigma(2:,1)/float(runNumber),sigma(:channel,2)
...
subroutine event_dSigma_dE_0pions(E,eMin,eMax,dE,string,runNumber,hists,initHists,makeOutputIn,&
...
!                   write(*,*)'  makeInitHists=.false.'
             ...
             if(makeInitHists) then
                call createHist(dsigma_dE, 'dSigma_0pions/dEkin('// trim(name) //&
                     & ') for single '//trim(name)// ' production' &
                     &  ,eMin ,eMax,dE)
                call createHist(dsigma_dE_1X, 'dSigma_0pions/dEkin('// trim(name) //&
                     & ') for 1 '//trim(name)// 'and X(including the same id but different charge)  production' &
                     &  ,eMin ,eMax,dE)
                call createHist(dsigma_dE_2X, 'dSigma_0pions/dEkin('// trim(name) //&
                     & ') for 2 '//trim(name)// 'and X(including the same id but different charge) production' &
                     &  ,eMin ,eMax,dE)
                call createHist(dsigma_dE_multi, 'dSigma_0pions/dEkin('// trim(name) //&
             ...
             if(sameFileName) then
                filename=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.dat'
                filename1X=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'_1X.dat'
                filename2X=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'_2X.dat'
                filenameMulti=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'_MULTI.dat'
             else
                filename=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.' //trim(intTochar(runNumber))//'.dat'
                filename1X=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.' //trim(intTochar(runNumber))//'_1X.dat'
                filename2X=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.' //trim(intTochar(runNumber))//'_2X.dat'
                filenameMulti=trim(string)//'_dSigma_0pions_dEkin_'// trim(name)&
                     & //'_charge_'//intTochar_pm(charge) //'.' //trim(intTochar(runNumber))//'_MULTI.dat'
             end if
       ...
       if  (  sum(E(j)%numberParticles(1,:)).eq.0  )   then   ! no pions
              ...
              !Find out whether there are any particles in the event with charge="charge" and ID="particleIDs(i):
               if(( E(j)%numberParticles(i,charge).eq.1).and.( sum(E(j)%numberParticles(i,:)).eq.1)) then
                   if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                      ...
                      !!write(*,*) 'Single-particle: event j=',j, '   charge=',charge, '  Ekin=',ekin
                   else
                      write(*,*) 'Error in event_dSigma_0pions_dE', E(j)%numberParticles(i,charge), 'Stop!'
                      stop
                   end if
                end if
                ...
                if(E(j)%numberParticles(i,charge).eq.1) then
                   if( event_GetParticle(E(j),particleIDs(i),charge,1,part)) then
                      ...
                      !!write(*,*) '1-particle-plus-X: event j=',j, '   charge=',charge, '  Ekin=',ekin
                   else
                      write(*,*) 'Error in event_dSigma_0pions_dE', E(j)%numberParticles(i,charge), 'Stop!'
                      stop
                   end if
                ...
                if(E(j)%numberParticles(i,charge).eq.2) then
                     do particle_number=1,2
                          if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                             ekin=part%momentum(0)-part%mass
                             call addHist(dsigma_dE_2X , ekin  ,part%perweight*E(j)%numberParticles(i,charge))
                              !!write(*,*) 'Multi-particle: event j=',j, '   charge=',charge, '  Ekin=',ekin
                           else
                            write(*,*) 'Error in event_dSigma_0pions_dE_multi', E(j)%numberParticles(i,charge), 'Stop!'
                            stop
                           end if
                     end do
                end if
                ...
                if(E(j)%numberParticles(i,charge).ge.1) then
                     do particle_number=1,E(j)%numberParticles(i,charge)
                          if( event_GetParticle(E(j),particleIDs(i),charge,particle_number,part)) then
                             ekin=part%momentum(0)-part%mass
                             call addHist(dsigma_dE_multi , ekin  ,part%perweight*E(j)%numberParticles(i,charge))
                              !!write(*,*) 'Multi-particle: event j=',j, '   charge=',charge, '  Ekin=',ekin
                           else
                            write(*,*) 'Error in event_dSigma_0pions_dE_multi', E(j)%numberParticles(i,charge), 'Stop!'
                            stop
