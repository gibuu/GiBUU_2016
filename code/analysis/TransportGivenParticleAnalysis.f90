...
!
! PURPOSE
! * This module does the analysis of the output of eventtype transportGivenParticle
!*************************************************************************************
...
module transportGivenParticleAnalysis
  ...
  logical, parameter :: plot_z_coord = .true. ! plot z coordinate instead of radius
...
contains
  ...
  subroutine transportGivenParticle_analyze(Particles,timestep,id_in,freq_in)
    ...
    if (initflag) then
      ...
      do k=0,Nsteps
          call CreateHist(SF(k), 'reconstructed spectral function at timestep '//intTochar(k),minmass,maxmass,0.005)
          call CreateHist2D(SF_vsRadius(k),'reconstructed spectral function at timestep '//intTochar(k), &
    ...
    if (mod(timestep,frequency)==0) then
       ...
       do i=lBound(Particles,dim=1),uBound(Particles,dim=1)
          do j=lBound(Particles,dim=2),uBound(Particles,dim=2)
             ...
             call AddHist(SF(k),abs4(Particles(i,j)%momentum),Particles(i,j)%perweight*Ninv)
          end do
       end do
       ...
       do i=lBound(Particles,dim=1),uBound(Particles,dim=1)
          do j=lBound(Particles,dim=2),uBound(Particles,dim=2)
             ...
             if (plot_z_coord) then
               call AddHist2D (SF_vsRadius(k), (/abs4(Particles(i,j)%momentum),particles(i,j)%position(3)/), &
                               Particles(i,j)%perweight*Ninv)
             else
               call AddHist2D (SF_vsRadius(k), (/abs4(Particles(i,j)%momentum),absPos(particles(i,j))/), &
                               &Particles(i,j)%perweight*Ninv)
             end if
          end do
        end do
       call writeHist2D_Gnuplot(SF_vsRadius(k), 10, file='TGP_SFvsRadius_'//trim(inttochar(k))//'.dat')
    end if
