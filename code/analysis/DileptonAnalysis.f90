...
contains
      ...
      case (rho)
        ...
        if (particle_source) then
          ...
          if (parents(2) > 0) then
             ...
             if (channel == 0) then
               print *, "wrong baryon in dilepton analysis: ", parents(1)
               stop
             end if
          else
      ...
      case (Delta:F37_1950)  ! baryon resonances
        ...
        if (particle_source) then
          ...
          if (all(fs(1:2)%id==0)) then
            print *,"warning: R->rhoN->e+e-N failed!"
            cycle
          else if (fs(1)%id/=rho .or. fs(1)%charge/=0) then
            print *,"error in Dilep_Decays: no rho! ", id, fs(1:2)%ID, ch, fs(1:2)%charge
            stop
          end if
          ! (2) do rho0 -> e+e-
          rmom = fs(1)%momentum
  ...
  subroutine CountParticles(t,pertPart)
    ...
    use particleDefinition, only: particle
    use IdTable, only: Delta,pion,eta,rho,phi,omegaMeson,photon,JPsi,P11_1440,F37_1950
