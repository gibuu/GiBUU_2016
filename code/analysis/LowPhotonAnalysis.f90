...
module LowPhotonAnalysis
  ...
  ! PURPOSE
  ! Cuts on the pi0-gamma invariant mass in GeV, being applied to all pi0-gamma spectra (except the mass spectrum).
  ...
  ! PURPOSE
  ! Sigma parameter for the exp. resolution smearing (width of the Gauss or Novosibirsk function in GeV).
  ...
  ! PURPOSE
  ! Skewness parameter tau of the Novosibirsk function (for exp. resolution smearing).
  ...
  ! PURPOSE
  ! Cut on the kinetic energy of neutral pions in the pi0gamma_analysis. Only pions with
  ! kinetic energies larger than this cutoff are used for the analysis.
  !***************************************************************************
 ...
 contains
  ...
  subroutine readinput
    ...
    NAMELIST /lowPhotonAnalysis/ outputEvents, outputEvents_onlyFree, &
         ...
         pi0gamma_analysis, pi0gamma_momcut, pi0gamma_masscut, pi0gamma_mombin, &
    ...
    if(kruscheOutput)     write(*,*) '  =>Cut for Krusche analysis=', kruscheAnalyse_cut
    ...
    if (pi0gamma_analysis) then
      write(*,'(A,1F7.4)') ' pi0 gamma momentum cut [GeV]               ?', pi0gamma_momcut
      write(*,'(A,2F7.4)') ' pi0 gamma mass cut [GeV]                   ?', pi0gamma_masscut
      write(*,'(A,1F7.4)') ' pi0 gamma momentum bin size [GeV]          ?', pi0gamma_mombin
      write(*,'(A,1F7.4)') ' pi0 gamma mass resolution: sigma [GeV]     ?', pi0gamma_massres_sigma
      write(*,'(A,1F7.4)') ' pi0 gamma mass resolution: tau             ?', pi0gamma_massres_tau
      write(*,'(A,1F7.4)') ' pi0 kinetic energy cut                     ?', Ekin_pi0_cut
    end if
  ...
  !  * logical, intent (in)                        :: finalFlag   ! .true. if it is the last call for one
  ...
  subroutine analyze_Photon(teilchen,finalFlag)
    ...
    !*Last Modifacation :oliver buss
    !*Date: 12/06/03
    !*Makes Output for photon induced processes
    use lowElectronAnalysis,only : lowElectron_Analyze
    use particleDefinition
    use initLowPhoton, only: getTwoPi,getVecMes,getResonances,getSinglePi, getPi0Eta
    ...
    if (getResonances().or.getSinglePi().or.getTwoPi().or.getpi0Eta()) call  lowElectron_Analyze(teilchen,finalFlag)
    if ((getResonances().or.getSinglePi().or.getTwoPi()).and.KruscheOutput) call  kruscheAnalyse(teilchen,finalFlag)
    if ((getResonances().or.getSinglePi().or.getTwoPi()).and.FissumOutput) call  fissumAnalyse(teilchen,finalFlag)
  ...
  ! * Events stemming from omega, rho or phi production are printed to seperate files 'FinalEvents.rho.dat', ...
  ! OUTPUT for "photonAnalyse"
  ! * Produces files called Photons_m2_vs_m3*.dat and Photons_dsigma_dm*.dat.
  ! * The first one contains the distribution of the invariant mass of all 3gamma combinations vs the inv. mass of all 2gamma combinations.
  ! * The second one contains the cross section dSigma/dm vs the mass of 3 gammas.
  ! PURPOSE
  ...
  subroutine photonFS_Analyse (p)
    ...
    use Electron_origin, only : le_isOmegaEvent,le_ispi0etaEvent, &
         & le_isRhoEvent, &
         & le_isPiPiEvent,le_isResEvent,le_isPhiEvent,le_whichOrigin
    use AnaEventDefinition
    use AnaEvent, only : event_init, event_dump, event_add, event_clear, event_pairPhotons
    use hist2Df90
    use histf90
    ...
    do i=lBound(p,dim=1),uBound(p,dim=1)
       do j=lBound(p,dim=2),uBound(p,dim=2)
          ...
          if(le_isOmegaEvent(first))   call event_add(events_omega(first)  , particlePointer)
          if(le_isRhoEvent(first))     call event_add(events_rho(first)    , particlePointer)
          if(le_isPhiEvent(first))     call event_add(events_phi(first)    , particlePointer)
          if(le_isPiPiEvent(first))    call event_add(events_pipi(first)   , particlePointer)
          if(le_isResEvent(first))     call event_add(events_res(first)    , particlePointer)
          if(le_ispi0etaEvent(first))  call event_add(events_pi0eta(first) , particlePointer)
       end do
    end do
    ...
    if (photonAnalyse) then
      call event_pairPhotons (events,        '',        dsigma_dm_threeGammas(0,:),Hist_massPhotons(0))
      call event_pairPhotons (events_rho,    '.rho',    dsigma_dm_threeGammas(1,:),Hist_massPhotons(1))
      call event_pairPhotons (events_omega,  '.omega',  dsigma_dm_threeGammas(2,:),Hist_massPhotons(2))
      call event_pairPhotons (events_phi,    '.phi',    dsigma_dm_threeGammas(3,:),Hist_massPhotons(3))
      call event_pairPhotons (events_pipi,   '.pipi',   dsigma_dm_threeGammas(4,:),Hist_massPhotons(4))
      call event_pairPhotons (events_res,    '.res',    dsigma_dm_threeGammas(5,:),Hist_massPhotons(5))
      call event_pairPhotons (events_pi0eta, '.pi0eta', dsigma_dm_threeGammas(6,:),Hist_massPhotons(6))
    end if


    ! (3) Dump all events to file:
    !     print ID, charge, perweight, position and momentum of all produced particles

    if (outputEvents) then
       write(*,*) 'Writing events to file'

      filename='FinalEvents.dat'
      call event_dump(current_run_number,events,filename,outputEvents_onlyFree)

      filename='FinalEvents.rho.dat'
      call event_dump(current_run_number,events_rho,filename,outputEvents_onlyFree)

      filename='FinalEvents.omega.dat'
      call event_dump(current_run_number,events_omega,filename,outputEvents_onlyFree)

      filename='FinalEvents.phi.dat'
      call event_dump(current_run_number,events_phi,filename,outputEvents_onlyFree)

      filename='FinalEvents.pipi.dat'
      call event_dump(current_run_number,events_pipi,filename,outputEvents_onlyFree)

      filename='FinalEvents.res.dat'
      call event_dump(current_run_number,events_res,filename,outputEvents_onlyFree)

      filename='FinalEvents.pi0eta.dat'
      call event_dump(current_run_number,events_pi0eta,filename,outputEvents_onlyFree)
    end if
  ...
  !  * type(particle), target, dimension(:,:) :: p           ! particle vector
  ! OUTPUT
  !  * Produces an invariant mass spectrum of all pi0 gamma pairs in the final state
  !    and writes it to a file called 'pi0gamma_dsigma_dm.dat'
  !*******************************************************************************************************************
  subroutine pi0gamma_analyze (p)
    use particleDefinition
    use particlePointerListDefinition
    use AnaEventDefinition
    use IDTable, only: pion, photon, omegaMeson
    use minkowski, only: abs4,abs3
    use anaEvent, only: event_init, event_add, event_clear
    use lowElectron, only : le_get_FirstEventRange
    use histf90, only: histogram, CreateHist, AddHist, WriteHist, WriteHist_Gauss, WriteHist_Novo
    ...
    type(histogram),save :: dsigma_dm, dsigma_dm_momcut, dsigma_dp, dsigma_dE, dsigma_dEkin, omega_excit_func, decDensity
    integer, dimension (1:2) :: firstEvents
    integer :: i,j,first
    real :: m,E_gamma,dE,pabs,E_tot,E_kin,dens
    ...
    if (.not. dsigma_dm%initialized) then
      ...
      write (title,'(A,F6.3,A)') 'dsigma/dm (pi0 gamma) in microbarn/GeV/A, |p| < ',pi0gamma_momcut,' GeV'
      call CreateHist (dsigma_dm_momcut, title, 0., 1., 0.001)
      call CreateHist (dsigma_dp,    'dsigma/dp (pi0 gamma) in microbarn/GeV/A',      0.,  2., pi0gamma_mombin)
      call CreateHist (dsigma_dE,    'dsigma/dE (pi0 gamma) in microbarn/GeV/A',      0.,  2., pi0gamma_mombin)
      call CreateHist (dsigma_dEkin, 'dsigma/dE_kin (pi0 gamma) in microbarn/GeV/A', -0.2, 2., pi0gamma_mombin)
    ...
    if (.not. omega_excit_func%initialized) then
      dE = getDeltaEnergy()
      call CreateHist (omega_excit_func, 'omega excitation function (including final state interaction) in microbarn/A', &
    ...
    do i=lBound(p,dim=1),uBound(p,dim=1)
      do j=lBound(p,dim=2),uBound(p,dim=2)
        ...
        first=pp%firstEvent
        ! Add particle to the event with its firstEvent index.
        if (pp%ID == pion .and. pp%charge == 0 .and. kineticEnergy(pp)>Ekin_pi0_cut) then
    ...
    do i=firstEvents(1),firstEvents(2)
      ...
      do
        ...
        do
          ...
          m     = abs4 (p_pi%V%momentum + p_gam%V%momentum)      ! invariant mass of pi0-gamma pair
          pabs  = abs3 (p_pi%V%momentum + p_gam%V%momentum)      ! absolute 3-momentum of pi0-gamma pair
          E_tot = p_pi%V%momentum(0) + p_gam%V%momentum(0)       ! total energy of pi0-gamma pair
          E_kin = E_tot - m                                      ! kinetic energy of pi0-gamma pair
          call addHist (dsigma_dm, m, p_pi%V%perweight/float(num_Runs_sameEnergy*num_Energies))
          if (m>pi0gamma_masscut(1) .and. m<pi0gamma_masscut(2)) then
            call addHist (dsigma_dp,    pabs,  p_pi%V%perweight/float(num_Runs_sameEnergy*num_Energies))
            call addHist (dsigma_dE,    E_tot, p_pi%V%perweight/float(num_Runs_sameEnergy*num_Energies))
            call addHist (dsigma_dEkin, E_kin, p_pi%V%perweight/float(num_Runs_sameEnergy*num_Energies))
            ! divide out energy weight to get a proper excitation function
            call addHist (omega_excit_func, E_gamma, p_pi%V%perweight/float(num_Runs_sameEnergy)*energyWeight(E_gamma))
            if (PIL_omegaDec_Get (p_pi%V%firstEvent, dens)) &   ! decay density
              call addHist (decDensity, dens/rhoNull, y=p_pi%V%perweight/float(num_Runs_sameEnergy*num_Energies))
            if (pabs<pi0gamma_momcut) then ! momentum cut
              call addHist (dsigma_dm_momcut, m, p_pi%V%perweight/float(num_Runs_sameEnergy*num_Energies))
            end if
          end if
          p_gam => p_gam%next
        end do
    ...
    call WriteHist_Gauss (dsigma_dm, "pi0gamma_dsigma_dm_gauss.dat", pi0gamma_massres_sigma)
    call WriteHist_Novo  (dsigma_dm, "pi0gamma_dsigma_dm_novo.dat",  pi0gamma_massres_sigma, pi0gamma_massres_tau)
    call WriteHist       (dsigma_dm_momcut, file="pi0gamma_dsigma_dm_momcut.dat")
    call WriteHist_Gauss (dsigma_dm_momcut, "pi0gamma_dsigma_dm_momcut_gauss.dat", pi0gamma_massres_sigma)
    call WriteHist_Novo  (dsigma_dm_momcut, "pi0gamma_dsigma_dm_momcut_novo.dat",  pi0gamma_massres_sigma, pi0gamma_massres_tau)
    !***************************************************************************
    ...
    ! include FSI effects.
    !***************************************************************************
    call WriteHist (omega_excit_func, mul=omega_excit_func%xBin, file="OmegaExcitFunc_pi0gamma.dat")  ! multiplicative factor prevents division by bin size
  ...
  subroutine kruscheAnalyse(p,finalFlag)
    ...
    integer, save         :: numberOfCalls=0 ! counts the number of calls per energy
    ...
    if(firstCall) then
       ...
       ! PURPOSE
       ! Same as KruscheAnalyse.dat, but output is written afer each run and not only
       ! the averaged result after each energy.
       !*******************************************************************
    ...
    if(initFlag) then
       ...
       error_cut=0.
       numberOfCalls=0
       call CreateHist(hist_deltaE, 'delta E according to Krusche',-0.4,0.4,0.005)
  ...
  contains
    ...
    ! Inputs
    ! * real, intent(in) :: E_gamma ! Energy of gamma in GeV in frame where nucleon rests
    !*************************************************************************************
    real function get_EPion(E_gamma)
  ...
  subroutine fissumAnalyse(p,finalFlag)
    ...
    type(histogram_mr),save,dimension(1:4)  :: dsigmadThetadT_pi ! Index 1: 51 deg, 2: 81 deg, 3: 109 deg, 4: 141 deg
    ...
    if(initFlag) then
       do i=1,4
          call CreateHist_mr(dsigmadThetadT_pi(i), 'dsigma/(dTheta dT_pion) [...b / degrees GeV] for piPlus according to &
               & Fissum PRC 53,3 (1996) at fixed angle'//realToChar(degs(i))//' degrees',0.,0.1,0.01)
       end do
       call CreateHist_mr(dsigmadTheta_greater17MeV, 'dsigma/dTheta  [...b / degrees] for piPlus according to &
    ...
    do i=lbound(p,dim=1),ubound(p,dim=1)
       do j=lbound(p,dim=2),ubound(p,dim=2)
          ...
          if(kineticEnergy(p(i,j)).gt.0.017) call AddHist_mr(dsigmadTheta_greater17MeV,theta_lab,p(i,j)%perweight,1.)

          ! dsigma/dOmega/dT_pi at fixed angles:
          channelLoop: do channel=1,4
             if(abs(theta_lab-degs(channel)).lt.5.) then
                call AddHist_mr(dsigmadThetadT_pi(channel),kineticEnergy(p(i,j)),p(i,j)%perweight/10.,1.)
                exit channelLoop
             end if
          end do channelLoop
       end do
  ...
  ! INPUTS
  ! *   type(particle), dimension(:,:), intent(in)  :: teilchen    ! particle vector
  ! *   logical, intent (in)                        :: finalFlag   ! .true. if it is the last call for one specific energy,
  !                                                                  therefore final output must be made.
  ...
  subroutine twoPi_output(teilchen, finalFlag,elab)
    ...
    logical ::  flag
    integer, save :: numCalls = 0 ! Counts how often the analysis routine is called before "finalFlag" is set. Therefore it counts the number of runs with the same energy.
    ...
    numCalls=numCalls+1 ! Counts how often the analysis routine is called before "finalFlag" is set. Therefore it counts the number of runs with the same energy.
    ...
    ensembleLoop :do ensemble=lBound(teilchen,dim=1),uBound(teilchen,dim=1)
       indexLoop : do index=lBound(teilchen,dim=2),uBound(teilchen,dim=2)
          ...
          if(flag) then
             ...
             ! ######### dsigma/dm
             m= massInv_twoPi(mom1,mom2 , teilchen(ensemble,index)%charge, teilchen(secondEnsemble,second)%charge)
   ...
   if(finalFlag) then
    ...
    ! They show  xsec (microbarn) for the  __two-pion events__   versus pion momentum (GeV)
    !
    ! Columns:
    ...
    ! PURPOSE
    ! The files are produced in the runs with eventtype=3=LowPhoton
    ! They show  xsec (microbarn) for the  __two-pion events__   versus radius of the nucleus
    ...
    ! PURPOSE
    ! The files are produced in the runs with eventtype=3=LowPhoton
    ! They show  xsec (microbarn) for the  __two-pion events__   versus pion momentum
    ! for various final states
    !
    ! Columns:
  ...
  function massInv_TwoPi(p,q,chargep,chargeq) Result(m)
    ...
    if(m.lt.(2*massPiZero)) then
       Print *,"Impulse=",p,q
       Print *,"Massen=",massP,massQ
       Print *,"Invariante Masse",m
       stop
    end if
    return
