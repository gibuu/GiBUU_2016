...
contains
  ...
  subroutine bzReadline (f, buf, length, debug)
    ...
    if (length>0) then
      ...
      if (err == 4) then
        f%eof = .true.
        if (present(debug) .and. debug) print *,"bzRead(1): EOF"
      else if (err/=0 .or. r<1) then
        print *,"bzRead(1): Error ",err," in file ",trim(f%fname)
        if (err<0) stop
      end if
    ...
    do i=length+1,len(buf)
      ...
      if (err == 4) then
        f%eof = .true.
        if (present(debug) .and. debug) print *,"bzRead(2): EOF"
      else if (err/=0 .or. r<1) then
        print *,"bzRead(2): Error ",err," in file ",trim(f%fname)
        if (err<0) stop
      end if
      buf(i:i) = c
  ...
  subroutine bzCloseR (f)
    ...
    if (err/=0) then
      print *,"Error closing bz file ",f%fname
      print *,"Error code: ",err
      stop
   end if
    ...
    err = fClose(f%handle)
    if (err /= 0) then
      print *,"Error closing file ",f%fname
      print *,"Error code: ",err
      stop
    end if
  ...
  subroutine bzCloseW (f)
    ...
    if (err/=0) then
      ...
      print *,"bzCloseW: writing file ",f%fname
      print *,nbytes_in," Bytes raw"
      print *,nbytes_out," Bytes compressed"
    end if
