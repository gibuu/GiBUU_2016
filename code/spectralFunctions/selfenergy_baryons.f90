...
contains
  ...
  subroutine readInput
    ...
    NAMELIST /selfenergy_realPart/ rel_accuracy, intSolver, makeTable, noDispersion, maxRes, minRes, &
                                   debugflag, extrapolateAbsP, writeLocal
...
!    real, intent (in)                :: rhoN,rhoP ! In fm^-3
    ...
    if(E_pole.gt.E) then
       ...
       case(cernlib)
          ...
          result_low=dCAUCH(integrand,A,B,E,abs_acc)
          a=E+(E_pole-E)/2.
          b=cutoff
          if(a.gt.b) then
             write(*,*) 'cutoff too small: a>b',a,b
          ...
          result_up=dCAUCH(integrand,A,B,E_pole,abs_acc)
       end Select
       ...
       case(cernlib)
          ...
          !          write(*,*) '1',a,b
          result_low=dCAUCH(integrand,A,B,E_pole,abs_acc)
          a=E_pole+(E-E_pole)/2.
          b=cutoff
          if(a.gt.b) then
          ...
          !          write(*,*) '2',a,b
          result_up=dCAUCH(integrand,A,B,E,abs_acc)
       end Select
