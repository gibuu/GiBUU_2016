...
contains
  ...
  subroutine coulombPropagation (teilchen)
    ...
    If (PrintParticleVectors) then
       ...
       Open(97,File="PertParticles_BeforeCoulomb_pos.dat")
       Open(98,File="PertParticles_BeforeCoulomb_mom.dat")
       Open(99,File="PertParticles_BeforeCoulomb.dat")
       write(97,*) "# position[fm],ID of particle, charge"
       write(98,*) "# 4-momentum[GeV],ID of particle,charge"
       write(99,*) "# ID of particle,charge,event, number, perturbative, antiparticle,lastCollisionTime,perweight"
       Do i=1,size(teilchen,dim=1)
          Do j=1,size(teilchen,dim=2)
             If (teilchen(i,j)%Id.ne.0)then
                Write(97,'(3F7.2,2I5)') teilchen(i,j)%position,teilchen(i,j)%ID,teilchen(i,j)%Charge
                Write(98,'(4F8.4,2I5)') teilchen(i,j)%momentum,teilchen(i,j)%ID,teilchen(i,j)%Charge
                Write(99,'(5I8,2L8,2F8.4)') teilchen(i,j)%ID,teilchen(i,j)%Charge,&
                     & teilchen(i,j)%event,teilchen(i,j)%Number, teilchen(i,j)%perturbative, &
                     & teilchen(i,j)%antiparticle, teilchen(i,j)%lastcollisionTime,teilchen(i,j)%perweight
             end if
          End do
       End do
       Close(97)
