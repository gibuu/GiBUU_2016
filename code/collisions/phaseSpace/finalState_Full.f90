...
CONTAINS
  ...
  contains
    ...
    subroutine getMaximum(maxBWD)
      ...
      else if (pairIn(1)%Id<=nres+1 .and. pairIn(2)%Id==0) then
    ...
    subroutine checkMaximum(value, maxValue)
      ...
      if (value>maxValue) then
         ...
         if ((.not. get_MediumSwitch_coll()) .and. (MaxNumberOfErrors<numberOfErrors)) then
            write (*,*) 'massass/checkMaximum: This problem occured now too often! Stop'
            stop
         end if
      end if
    ...
    subroutine throwDice (success, momentum, mass, massFailure_Nuc, treatNuc_onshell)
      ...
      If (debugFlag) Print *, flagStable, IdOut, minMass
...
!  Testing:
      ...
      do k=1,n
         if(.not.flagStable(k)) then
            ...
            If(isMeson(finalState(k)%Id)) then
               gamtot=WidthMesonMedium(finalState(k)%Id, mass(k), plrf(0:3), mediumAtCollision)
            else
               gamtot=WidthBaryonMedium(finalState(k)%Id, mass(k), plrf(0:3), mediumAtCollision)
            end if
  ...
  contains
     ...
     do i = 1,2000
        ...
        call lorentz(-betaToCM, plrf, 'massass_nBody(1)')       ! boost from CM to comput. frame
        call lorentz(betaToLRF, plrf, 'massass_nBody(2)')       ! boost from comput. frame to LRF
        gamtot=WidthMesonMedium(rho,mass,plrf(0:3),mediumAtCollision)
