...
contains
  ...
  subroutine readInput
    ...
    ! * NN_NR_noniso
    !**********************************************************************
    NAMELIST /angular_distribution/ deltaPWave, debug, rho_pipi_nonIsotropic, NNisotropic, iParam_gammaNVN, &
  ...
  function winkel (teilchenIn, teilchenOut, srts, betaToCM, mediumAtCollision, successflag) result (pscatt)
    ...
    type(particle),dimension(1:2),intent(in) :: teilchenIn,teilchenOut
    real, intent(in) :: srts
    ...
    if (id2 == 0) then
       ...
       If (id1==Delta .and. id3==pion .and. deltaPWave) then
          ...
          pscatt = pscatt_Delta_PWave (teilchenIn(1), srts)
       ...
       if ((iii2==pion .and. iii1==nucleon) .and. &
          ...
          pscatt = pscatt_piN_piN_backward (teilchenIn, srts)
          ...
          pscatt = pscatt_VN_VX (teilchenIn, id3, id4, m3, m4, srts, mediumAtCollision, successVN)
          ...
          ! gamma N -> V N
          pscatt = pscatt_gammaN_VN (teilchenIn, id3, id4, m3, m4, srts, mediumAtCollision)

       else if (isBaryon(iii1) .and. isBaryon(iii2)) then

          if (debug) write(*,*) ' Bar Bar -> Bar Bar'
          pscatt = pscatt_BarBar (teilchenIn, teilchenOut, srts, betaToCM, successND)
...
!    call WriteParticle(6,1,2, teilchenIn(2))
    ...
    massMes = massMes + vecMes_massShift(idMes,mediumAtCollision%density)
    ...
    if (isMeson(teilchenIn(1)%ID)) then
      pscatt = rotateTo (teilchenIn(1)%momentum(1:3), pscatt)
    else
      pscatt = rotateTo (teilchenIn(2)%momentum(1:3), pscatt)
    end if
...
!    write(*,'(0P,3f11.3)') pscatt
  ...
  function pscatt_gammaN_VN (teilchenIn, id3, id4, mass3, mass4, srts, mediumAtCollision) result (pscatt)
    ...
    if (teilchenIn(1)%ID==photon) then
      photonMom = teilchenIn(1)%momentum
    else
      photonMom = teilchenIn(2)%momentum
    end if
  ...
  function pscatt_piN_piN_backward (teilchenIn, srts) result (pscatt)
    ...
    If (debug) then
       write(77,*) cost
       write(34,*) srts, teilchenIn(1)%Id, teilchenIn(2)%id
    end if

    ! Rotate according to incoming pion
    if (teilchenIn(1)%ID == pion) then
      pscatt = rotateTo (teilchenIn(1)%momentum(1:3), pscatt)
    else
      pscatt = rotateTo (teilchenIn(2)%momentum(1:3), pscatt)
    end if
  ...
  function pscatt_BarBar (teilchenIn, teilchenOut, srts, betaToCM, successflag) result(pscatt)
    ...
    if (prcm==0.) then
       ...
       write(*,*) id1,m1,teilchenIn(1)%momentum, &
                  id2,m2,teilchenIn(2)%momentum
       stop
    end if
    ...
    if (id1==Nucleon .and. id2==Nucleon .and. id3==Nucleon .and. id4==Nucleon) then
       ...
       if (NNisotropic .and. .not.(teilchenIn(1)%antiParticle .or. teilchenIn(2)%antiParticle)) then
       ...
       if (teilchenIn(1)%antiParticle.eqv.teilchenIn(2)%antiParticle) then
          ...
          case (1)
             ...
             if (teilchenIn(1)%charge+teilchenIn(2)%charge==1) then
                ! pn
          ...
          if (teilchenOut(1)%charge==teilchenIn(1)%charge .or. teilchenOut(1)%charge==teilchenIn(2)%charge) then
          ...
          if (teilchenOut(1)%antiParticle.neqv.teilchenIn(1)%antiParticle) c1=-c1
