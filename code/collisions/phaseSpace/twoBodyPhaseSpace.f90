...
module twoBodyPhaseSpace
  ...
  public :: Integrate_2bodyPS_resonance, nnRR, setMaxSqrts

  logical, parameter :: DoWrite = .false. ! switch on/off printig of data files

  integer, save :: srts_maxIndex = 220  ! this will be overwritten by 'setMaxSqrts', according to the high energy thresholds in master_2body
...
contains
  ...
  function Integrate_2bodyPS_resonance (resID, srts, massStable, scalarPotential) result (ps)
    ...
    if ((abs(massStable-mN) < 0.001 .or. abs(massStable-hadron(eta)%mass) < 0.001) .and. (abs(scalarPotential) < 0.001)) then
       ...
       If (down<0 .or. down>srts_maxIndex-1 .or. resID<resID_min .or. resID>resID_max) then
          ...
          !          write(*,*) 'WARNING : Out of bounds. Slow speed in Integrate_2bodyPS_resonance'
       else
  ...
  contains
    ...
    subroutine initialize
      ...
      integer :: srtsIndex,resonanceIndex,j
      real :: srtsDummy,stableDummy
      ...
      if (DoW) write(*,*) 'Writing Output to TwobodyPS_R_*.dat. where * is the ID of the resonance'

      allocate(ps_Field(1:2,0:srts_maxIndex,resID_min:resID_max,1:5))

      do j=1,2
         ! Choose different mass of the stable particle
         If(j==1) then
            stableDummy=mN
         else if (j==2) then
         ...
         do resonanceIndex = resID_min, resID_max
            if (DoW) Open(100,file='TwobodyPS_R_'//intToChar(resonanceIndex)//'.dat')
            if (DoW) write(100,*) '#sqrt(s),integral, maximal contribution,i'
            do srtsIndex=0, srts_maxIndex
               srtsDummy=float(srtsIndex)*delta_Srts+srtsMin
               psDummy = Calculate(resonanceIndex, srtsDummy, stableDummy, 0.)
               ! Save output
               ps_Field(j,srtsIndex,resonanceIndex,1:5)=psDummy
               if (DoW) write(100,'(8g12.5)') srtsDummy, psDummy, stableDummy
            End do
            if (DoW) close(100)
         end do
      end do
    ...
    subroutine initialize
      ...
      do id = lBound(integral_Field,dim=1), uBound(integral_Field,dim=1)
         do i = 0, srts_maxIndex
            ...
            if (DoW) write(100,'(F7.3,2I4,F7.3)') srtsDummy,id,i,integral_Field(id,i)
         end do
      end do
      if (DoW) close(100)
