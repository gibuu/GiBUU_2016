...
!
! PURPOSE
! This module collects some routines for cross section parametrisations
...
!   end function stotphi
  ...
  subroutine calcXS_gammaN2strange(srts,sigma)
      ...
      use IdTable, only : kaon, lambda,SigmaResonance
      use particleProperties, only: hadron
  ...
  ! NOTES
  ! The matrix element for the process is tabulated in the file "gammaN_omegaN_ME_saphir.dat".
  ! It is computed from the (splined) SAPHIR cross sections.
  !***************************************************************************************************
  subroutine calcXS_omega_saphir(srts,cs)
    use IdTable, only: omegaMeson
    use particleProperties, only: hadron
