...
contains
  ...
  ! (1) Check  Input
  if ((teilchenIn(1)%Id==pion .or. teilchenIn(1)%Id==eta) &
      .and.teilchenIn(2)%Id>=Lambda .and. teilchenIn(2)%Id<=omegaResonance) then
    meson_particle   = teilchenIn(1)
    hyperon_particle = teilchenIn(2)
  else if ((teilchenIn(2)%Id==pion .or. teilchenIn(2)%Id==eta) &
  ...
  ! (5) Check Output
  If (Sum(teilchenOut(:)%Charge) /= hyperon_particle%charge+meson_particle%charge) then
  ...
  !  real, dimension(-4:4,-1:1,0:nssmmax), save ::  ssigka,ssigka_res
    ...
    subroutine makeDecision
      ...
      if (sigmaRes_tot >= cut) then
        ...
        teilchenOut(1:2)%Id = (/ kaonBar, nucleon /)
        if (totalCharge==1) then
          ...
          if (hyperon_particle%Id==SigmaResonance) then
            ...
            teilchenOut(1:2)%Charge = (/ qkaon, -qkaon /)
          else
            x=rn()
            if (x<=0.5) then
