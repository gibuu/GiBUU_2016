...
contains
  ...
  ! NAME
  ! subroutine decideOnAnnihilation(teilchenReal,time,flag,widthAverageOut,widthTotalAverageOut,
  ...
  ! INPUTS
  ! * type(particle), dimension(:,:) :: teilchenReal   ! real particle vector
  ...
  subroutine decideOnAnnihilation(teilchenReal,time,flag,widthAverageOut,widthTotalAverageOut,&
    ...
    type(particle), intent(in), dimension(:,:) :: teilchenReal   ! real particle vector
    real, intent(in)                           :: time           ! actual time step
    logical, intent(OUT):: flag                                  ! if .true. --- do annihilation
    real, optional, intent(OUT) :: widthAverageOut               ! annihilation width of an antiproton
    ! averaged over all parallel ensembles
    real, optional, intent(OUT) :: widthTotalAverageOut          ! total width of an antiproton
    ! averaged over all parallel ensembles
    real, optional, intent(OUT) :: baryonDensityAverageOut       ! average density of baryons
    real, optional, intent(OUT) :: sigmaAnniAverageOut           ! average annihilation cross section
    real, optional, intent(OUT) :: vRelAverageOut                ! average relative velocity
    real, optional, intent(OUT) :: srtS_vacuumAverageOut         ! average vacuum c.m. energy
    ...
    numberEnsembles=size(teilchenReal,dim=1)
    numberParticles=size(teilchenReal,dim=2)
    ...
    ensemble_loop : do ensemble=1,numberEnsembles
       ...
       index_loop :   do index=1,numberParticles
          ...
          If(.not.teilchenReal(ensemble,index)%antiparticle) cycle index_loop

          If(isMeson(teilchenReal(ensemble,index)%Id)) then
          ...
          call  gamma(teilchenReal(ensemble,index),time,width,widthTotal,baryonDensity,sigmaAnni,&
               &vRel,srtS_vacuum)
  ...
  contains
    ...
    subroutine gamma(antiBaryon,time,width,widthTotal,baryonDensityOut,sigmaAnniOut,vRelOut,srtS_vacuumOut)
      ...
      integer, parameter :: imode=2     ! 1 --- use point particle density,
      ...
      ensemble_loop : do ensemble=1,numberEnsembles
         ...
         index_loop :   do index=1,numberParticles
            ...
            If( isMeson(teilchenReal(ensemble,index)%Id) .or.&
                 &teilchenReal(ensemble,index)%antiparticle ) cycle index_loop

            d2=dot_product( antiBaryon%position(1:3)-teilchenReal(ensemble,index)%position(1:3), &
                 & antiBaryon%position(1:3)-teilchenReal(ensemble,index)%position(1:3) )
            ...
            if(baryonNumber.le.maxBaryonNumber) &
                 & BaryonsFound(baryonNumber)=teilchenReal(ensemble,index)
      ...
      if(sigmaTotal .lt. sigmaAnni) then
         write(6,*)'srtS_vacuum, sigmaTotal,sigmaAnni:',srtS_vacuum,sigmaTotal,sigmaAnni
      end if

      sigmaNonAnni=sigmaTotal-sigmaAnni
      ...
      loop_over_trials : do nloop=1,10 ! Upper limit must be set more than nTrials
         ...
         If(.not.HiEnergyFlag) then  !**** NOT HIGH ENERGY ****
            ...
            call setKinematics(srtS,srtS_vacuum, betaToLRF,betaToCM,&
                 mediumAtCollision,(/antiBaryon,Baryon/),&
         ...
         flag = checkPauli(finalState,teilchenReal)
    ...
    subroutine gamma_exact(antiBaryon,time,width)
      ...
      ensemble_loop : do ensemble=1,numberEnsembles
         ...
         index_loop :   do index=1,numberParticles
            ...
            Baryon=teilchenReal(ensemble,index)   ! Baryon is selected
            ...
            call sigmaBarAntiBar(srtS_vacuum,(/antiBaryon,Baryon/),mediumATcollision,sigTotal=sigmaTotal,&
  ...
  ! INPUTS
  ! * type(particle), intent(inOUT), dimension(:,:) :: teilchenReal     ! real particle vector
  ! * real, intent(in)                              :: time             ! actual time step
  ! * real, intent(in)                              :: annihilationTime ! time when annihilation is enforced
  ! OUTPUT
  ! * type(particle), intent(inOUT), dimension(:,:) :: teilchenReal   ! real particle vector changed
  ! NOTES
  ! Presently suitable for the real particles and for the parallel ensemble mode.
  !*************************************************************************
  subroutine DoAnnihilation(teilchenReal,time,annihilationTime)
    ...
    type(particle), intent(inOUT), target, dimension(:,:) :: teilchenReal   ! real particle vector
    real, intent(in)                                      :: time           ! actual time step
    real, intent(in)                                      :: annihilationTime ! time when annihilation is enforced
    ...
    if(.not.flagAnni .or. time-timePrevious.lt.1.e-06) then
       call decideOnAnnihilation(teilchenReal,time,flagAnni,width,widthTotal,baryonDensity,&
    ...
    ensemble_loop : do ensemble=1,numberEnsembles
       ...
       index_loop :   do index=1,numberParticles
          ...
          If(.not.isBaryon(teilchenReal(ensemble,index)%Id)) then
          ...
          antibar=>teilchenReal(ensemble,index)
          ...
          index1_loop :   do index1=1,numberParticles
             ...
             If(teilchenReal(ensemble,index1)%ID < 0) exit index1_loop !!! ID==-1
             If(teilchenReal(ensemble,index1)%ID <= 0) cycle index1_loop

             If(      teilchenReal(ensemble,index1)%antiparticle &
                  & .or. isMeson(teilchenReal(ensemble,index1)%Id) ) cycle index1_loop

             d2=dot_product( antibar%position(1:3)-teilchenReal(ensemble,index1)%position(1:3), &
                  & antibar%position(1:3)-teilchenReal(ensemble,index1)%position(1:3) )
          ...
          if(index_min.ne.0) then
             ! Baryon is selected
             bar=>teilchenReal(ensemble,index_min)
          else
          ...
          do i=1,nFinal
             ...
             If(particlePropagated(finalState(i))) then
                ...
                If(.not.setFlag) then
                   ...
                   write(*,*) size(finalState), lBound(teilchenReal), uBound(teilchenReal)
    ...
    subroutine analyse
      ...
      do ensemble=1,numberEnsembles
         ...
         index_loop :   do index=1,numberParticles
            ...
            if( teilchenReal(ensemble,index)%ID.eq.pion ) Npion=Npion+1
