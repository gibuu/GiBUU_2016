...
contains
  ...
  subroutine readInput
    ...
    !*********************************
    NAMELIST /master_1Body/ correctEnergy, debug, StableInFormation, omegaDecayMediumInfo, omegaDecay_restriction
  ...
  subroutine decayParticle (resonanceIN, finalState, collisionFlag, pauliFlag, finalFlag, time, gammaOut)
    ...
    if (.not. getRMF_flag()) then
       ...
       momentumLRF = resonance%momentum
       betaToLRF = 0.
    ...
    If (isMeson(resonance%ID)) then
      ...
      call decayWidthBaryonMedium (resonance%ID,resonance%mass,momentumLRF,mediumATposition,width,pauliFlag)
    else
    ...
    ! one event is possible per timestep. Thus it is more consistent, to
    ! give the probability by the second equation, which again only holds for
    ! very small timesteps...
    wahrscheinlichkeit=1.-exp(-delta_T/gamma * gammaDecay/hbarc)      !    decay probability
...
!!$    end do
  ...
  function decayParticle_rhoN (resonanceIN, finalState) result (gammaDecay)
    ...
    use IDTable, only: isMeson, isBaryon, getAntiMeson, rho, nucleon, photon
    use particleDefinition
    ...
    ! (3) Evaluate the decay width of the particle
    call decayWidthBaryonMedium (resonance%ID, resonance%mass, momentumLRF, mediumAtPosition, width, success)
    ...
    if (dID==0) then
      print *, "problems in decayParticle_rhoN!"
      print *, width(:)
      print *, resonance%ID, resonance%charge, antiParticleFlag
      print *, resonance%mass, abs4(resonance%momentum)
      call traceback()
    end if
...
!     end if
  ...
  subroutine assignCharge(outPart,inID,inCharge)
    ...
    if (inID==dSStar_minus .or. inID==dSStar_plus) then
       if (outPart(1)%ID == inID-2) then
          ...
          stop 'Strange outgoing particle in dsStar decay'
       endif
       return
    endif
    ...
    if ((inID==dStar .or. inID==dStarBar).and.(Incharge==0)) then
       if (outPart(1)%ID == dMeson.or.outPart(1)%ID == dbar) then
          ...
          write(*,*) 'assignCharge:',inID,outPart(1)%ID,outPart(2)%ID
          stop 'Strange outgoing particle in dStar decay'
       endif
       return
    endif
  ...
  !**********************************************************************************
  subroutine getMomentum_and_Medium(resonance,momentumLRF, betaToLRF, mediumAtDecay)
  ...
  function setKinematics (resonance, finalState, mediumAtCollision, betaToLRF, L) result(collisionFlag)
    ...
    type(particle), intent(in)       :: resonance ! resonance which decays into "FinalState"
    type(medium), intent(in)         :: mediumAtCollision ! medium information
    real, dimension(1:3), intent(in) :: betaToLRF ! beta for boost to Local Rest Frame
    integer, intent(in)              :: L         ! angular momentum in final state
    type(particle), dimension(:)     :: finalState
    logical                          :: collisionFlag ! "true" if kinematics could be set, "false" if not
    ...
    Case(2)
       energyCorrectLoop : Do i=1, maxCorrectLoop
          ...
          call massass(srts_vacuum,mediumAtCollision,pair,finalState,betaToLRF,betaToCM,L,flag)
          If (.not. flag) then
          ...
          If (correctEnergy) then
             ...
             call energyCorrection(srts,betaToLRF,betaToCM, mediumAtCollision, finalState, successFlag)
...
!            If (debug) write (*,*) 'initial srts=', sqrts(Pair(1),Pair(2))
    ...
    Case(3)
       Do i=1, maxCorrectLoop
          ...
          call assMass(srts_vacuum,mediumAtCollision,pair,finalState,spotOut3,betaToLRF,betaToCM,flag)
          If (.not. flag) then
